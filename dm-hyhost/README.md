...
provides the host-side FTL for the HYHOSTSSD FEMU device
(`femu_mode=5`). It stacks on `/dev/nvme0n1` inside the guest VM and
exposes `/dev/mapper/hyhost0`, splitting the namespace into a
...
FEMU; the guest sees `/dev/nvme0n1` and stacks dm-hyhost on it.

```
   F2FS / ZenFS / RocksDB / fio
              │
              ▼
      /dev/mapper/hyhost0          ← dm-hyhost (this module)
  ┌───────────────────────┬─────────────────────────────┐
...
./scripts/build.sh                       # → dm-hyhost.ko  (KDIR= to override)
sudo ./scripts/load.sh                    # rmmod (if loaded) + insmod
sudo ./scripts/setup.sh                   # /dev/mapper/hyhost0, R = whole device
#   sudo ./scripts/setup.sh /dev/nvme0n1 33554432 hyhost0   # R = 16 GiB
...
```
0 <len> hyhost <backing_dev> <r_end> [max_r_end_lba]
```
...
| | value |
|---|---|
| host page | 4 KiB (`HYHOST_PAGE_SHIFT=3`; F2FS/RocksDB/fio block size) |
| NAND block | 2 MiB = 512 host pages (`HYHOST_PGS_PER_BLK`) |
| channels × LUNs | 8 × 4 = 32 |
| **line** | 64 MiB = 16,384 host pages (`HYHOST_LINE_PBLOCKS`) |
| **zone** | 1 GiB = 16 lines (`HYHOST_ZONE_PAGES` = 262,144) |
...
the high watermark. Two victim policies (`dmsetup message hyhost0 0
gc_policy {greedy|cb}`):
...
`dmsetup message hyhost0 0 gc` forces a sweep.
...
```
0 <len> hyhost r_end=<lba> pages=<n> lines=<n> line_pblocks=<n>
...
sudo dmsetup message hyhost0 0 set_r_end <NEW_R_END_LBA>
```
...
- **Grow** (R↑): the device validates that the reshaped S-zones are all
  ...
  re-exposed onto the free ring (`hyhost_quiesce_grow`).
...
R-zone count. dm-hyhost **answers this locally from its own `r_end`**
...
4. dm-hyhost, max-provisioned via `max_r_end_lba`, runs the shrink/grow
...
```bash
sudo ./scripts/f2fs_setup.sh [BACKING] [R_END_SECTORS] [MNT]   # default 8 GiB, /mnt/hyhost
...
**ZenFS / RocksDB** stack on the same `/dev/mapper/hyhost0` with an F2FS
...
| `gc_lat_hyhostr.sh` | host-FTL line-GC relocation latency |
...
- `dm-hyhost.ko` needs `linux-headers` matching `uname -r` exactly.
