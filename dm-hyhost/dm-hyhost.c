...
/*
 * dm-hyhost - host-managed FTL for the HYHOSTSSD FEMU device.
 *
 ...
 *
 *   echo "0 <sectors> hyhost <backing_dev> <r_end_lba> [max_r_end_lba]" \
 *       | dmsetup create hyhost0
 *
 ...
 * (see hyhost_ctr). The boundary moves at runtime via `dmsetup message ...
 ...
 * is left untouched.
 *
 * `dmsetup status` emits one key=value line; see hyhost_status() for the
 * field list.
 */
...
#define DM_MSG_PREFIX "hyhost"

#define HYHOST_PAGE_SHIFT   3U                          /* 4 KiB / 512B */
#define HYHOST_PAGE_SECTORS (1U << HYHOST_PAGE_SHIFT)
#define HYHOST_PAGE_MASK    (HYHOST_PAGE_SECTORS - 1)
#define HYHOST_PAGE_BYTES   (HYHOST_PAGE_SECTORS << SECTOR_SHIFT)
#define HYHOST_INVALID_PBA  ((sector_t)~0ULL)
#define HYHOST_INVALID_LPA  ((sector_t)~0ULL)
 ...
 *   host page      = 4 KiB  (HYHOST_PAGE_SHIFT=3; L2P granularity)
 ...
 *
 * HYHOST_PGS_PER_BLK is measured in *host* pages (2 MiB / 4 KiB = 512) so all
 * dm bookkeeping stays in the 4 KiB L2P unit; the 16 KiB device page only
 * matters to FEMU's timing model. */
#define HYHOST_NCHS         8U
#define HYHOST_LUNS_PER_CH  4U
#define HYHOST_PGS_PER_BLK  512U                              /* 2 MiB block / 4 KiB host page */
#define HYHOST_BLOCK_BYTES  (HYHOST_PGS_PER_BLK * HYHOST_PAGE_BYTES)
#define HYHOST_LINE_PBLOCKS                                   \
	(HYHOST_NCHS * HYHOST_LUNS_PER_CH * HYHOST_PGS_PER_BLK)   /* 16,384 host pages = 64 MiB */

/* Logical zone: the unit F2FS/ZenFS see and the granularity r_end moves on.
 * Must equal FEMU's hyhost_zone_size_mb (default 1 GiB) and be an integer
 * number of lines. In host pages: 1 GiB / 4 KiB = 262,144. */
#define HYHOST_ZONE_BYTES   (1ULL << 30)                     /* 1 GiB logical zone */
#define HYHOST_ZONE_PAGES   (HYHOST_ZONE_BYTES >> (SECTOR_SHIFT + HYHOST_PAGE_SHIFT))
 ...
 * allocation. Initialized to op=HYHOST_OP_NONE in hyhost_map(); the
 * R-region write path sets op=HYHOST_OP_R_WRITE and the rest of the
 ...
 * degrades past ~64. */
#define HYHOST_GC_BATCH      512U  /* max batch = gc_items[] allocation (runtime-knob cap);
                                    * pins one 4 KiB page per slot -> 2 MiB/target. */
#define HYHOST_GC_BATCH_DEFAULT 128U /* autonomous GC default: the drain knee. Background GC
                                      * runs under free-line pressure (a writer is stalling),
                                      * so max clean drain wins. Runtime: `dmsetup message
                                      * gc_batch`. */
#define HYHOST_GC_BATCH_SHRINK  32U  /* shrink force-GC cap: a background resize must not hurt
...
#define HYHOST_GC_SCAN_CHUNK 8192U /* rmap slots scanned per lock hold (>= BATCH) */
#define HYHOST_GC_VICTIM_MAX_VPC 95U /* victim valid% above this = useless (net gain ~0) -> park GC */

struct hyhost_mig_ctx {
	atomic_t          pending;
	struct completion done;
};

struct hyhost_mig_item {
	sector_t               lpage;
	sector_t               old_pblock;
	sector_t               new_pblock;
	struct page           *page;
	bool                   failed; /* IO error in either wave */
	struct hyhost_mig_ctx *ctx;
};
...
struct hyhost_c {
	...
	unsigned int        line_pblocks;      /* = HYHOST_LINE_PBLOCKS (64 MiB) */
	unsigned int        zone_pages;        /* = HYHOST_ZONE_PAGES (1 GiB); r_end
	...
	enum {
		HYHOST_GC_GREEDY = 0,
		HYHOST_GC_COST_BENEFIT,
	}                   gc_policy;

	/* Reverse map: pblock -> lpage (or HYHOST_INVALID_LPA if the pblock
	 ...
	 * (ch, lun) block making up the line (HYHOST_LINE_PBLOCKS /
	 * HYHOST_PGS_PER_BLK = 32 blocks per line at 8x4, one block per lane).
	 ...
	 * hyhost_line_erase (zsa=0x23) per line, then pushes onto free_lines
	...
	/* Batch migration buffers (HYHOST_GC_BATCH slots + data pages).
	 * Shared by the GC kthread and the shrink-force-GC path (zmod
	 * worker) - gc_io_lock serializes whole-line GC runs. */
	struct hyhost_mig_item *gc_items;
	u32                 gc_batch;   /* runtime GC migration batch cap (<=HYHOST_GC_BATCH);
...
static int hyhost_line_pop(struct hyhost_c *hc, sector_t *out)
{
	sector_t line;
...
static void hyhost_line_push(struct hyhost_c *hc, sector_t line)
{
	size_t tail;
	...
	if (line >= hc->push_bound_lines) {
		 ...
		 * hyhost_shrink_restore() on a failed shrink; dropped at the
...
static bool hyhost_line_in_dirty(struct hyhost_c *hc, sector_t line)
{
	size_t i;
...
static void hyhost_line_push_dirty(struct hyhost_c *hc, sector_t line)
{
...
static int hyhost_line_pop_dirty(struct hyhost_c *hc, sector_t *out)
{
	if (!hc->dirty_count)
		return -ENOSPC;
 ...
 * hc->lock. */
static inline bool hyhost_line_is_open(struct hyhost_c *hc, sector_t line)
{
...
static int hyhost_line_erase(struct hyhost_c *hc, sector_t line)
{
	...
	if (!bio)
		return -ENOMEM;
	line_lba = (line * hc->line_pblocks) << HYHOST_PAGE_SHIFT;
...
static int hyhost_erase_thread(void *data)
{
	struct hyhost_c *hc = data;
	unsigned long flags;
	...
	while (!kthread_should_stop()) {
		...
		have = (hyhost_line_pop_dirty(hc, &line) == 0);
		 ...
		 * its 32-block span. */
		hyhost_line_erase(hc, line);

		spin_lock_irqsave(&hc->lock, flags);
		hyhost_line_push(hc, line);
		if (READ_ONCE(hc->gc_blocked))
			pr_info("dm-hyhost: [GC] unparked (line %llu recycled)\n",
				(unsigned long long)line);
 ...
 */
static void hyhost_discard_range(struct hyhost_c *hc, sector_t lba,
				 unsigned int n_sectors, sector_t r_end)
{
	...
	lpage = (lba + HYHOST_PAGE_SECTORS - 1) >> HYHOST_PAGE_SHIFT;
	last_page = end >> HYHOST_PAGE_SHIFT;
	...
	while (lpage < last_page) {
		...
		for (; lpage < batch_end; lpage++) {
			...
			if (p == HYHOST_INVALID_PBA)
				continue;
			line = p / hc->line_pblocks;
			hc->l2p[lpage] = HYHOST_INVALID_PBA;
			hc->rmap[p]    = HYHOST_INVALID_LPA;
			BUG_ON(hc->line_valid[line] == 0);
			hc->line_valid[line]--;
			freed++;
			if (hc->line_valid[line] == 0 &&
			    !hyhost_line_is_open(hc, line)) {
				hyhost_line_push_dirty(hc, line);
	...
	if (freed) {
		...
		if (READ_ONCE(hc->gc_blocked))
			pr_info("dm-hyhost: [GC] unparked (discard freed %llu pages)\n",
...
static void hyhost_quiesce_shrink(struct hyhost_c *hc, sector_t new_r_pages)
{
	sector_t old_r_pages = hc->r_end >> HYHOST_PAGE_SHIFT;
	...
	for (i = 0; i < new_r_lines; i++) {
		if (hc->line_valid[i] != 0)
			continue;
		if (hyhost_line_is_open(hc, i))
			continue;
		if (hyhost_line_in_dirty(hc, i))
			continue;
...
static void hyhost_quiesce_grow(struct hyhost_c *hc, sector_t new_r_pages)
{
	sector_t old_r_lines = (hc->r_end >> HYHOST_PAGE_SHIFT) /
			       hc->line_pblocks;
	sector_t new_r_lines = new_r_pages / hc->line_pblocks;
	sector_t i;
 ...
 * headroom). */
static void hyhost_set_watermarks(struct hyhost_c *hc, sector_t active_lines)
{
...
static int hyhost_gc_items_alloc(struct hyhost_c *hc)
{
	unsigned int i;

	hc->gc_items = kcalloc(HYHOST_GC_BATCH, sizeof(*hc->gc_items),
			       GFP_KERNEL);
	if (!hc->gc_items)
		return -ENOMEM;
	for (i = 0; i < HYHOST_GC_BATCH; i++) {
...
static void hyhost_gc_items_free(struct hyhost_c *hc)
{
	unsigned int i;
...
static void hyhost_mig_end_io(struct bio *bio)
{
	struct hyhost_mig_item *it  = bio->bi_private;
	struct hyhost_mig_ctx  *ctx = it->ctx;
...
static void hyhost_mig_wave(struct hyhost_c *hc, unsigned int n,
			    unsigned int op)
{
	struct hyhost_mig_ctx ctx;
	...
	blk_start_plug(&plug);
	for (j = 0; j < n; j++) {
		struct hyhost_mig_item *it = &hc->gc_items[j];
		...
		bio->bi_iter.bi_sector = pblock << HYHOST_PAGE_SHIFT;
		bio_add_page(bio, it->page, HYHOST_PAGE_BYTES, 0);
		it->ctx         = &ctx;
		bio->bi_private = it;
		bio->bi_end_io  = hyhost_mig_end_io;
 ...
 * HYHOST_INVALID_PBA if no useful victim (no line has valid pblocks
 ...
 */
static sector_t hyhost_select_victim(struct hyhost_c *hc)
{
	sector_t best_line = HYHOST_INVALID_PBA;
	sector_t l;

	if (hc->gc_policy == HYHOST_GC_COST_BENEFIT) {
	...
	/* HYHOST_GC_GREEDY (default): smallest valid count wins. */
	{
 ...
 * here). Used by both the policy-driven path (hyhost_do_gc_one_line) and
 * the shrink force-GC path (hyhost_prepare_shrink_force_gc), which needs
...
static int hyhost_do_gc_one_line_at(struct hyhost_c *hc, sector_t victim,
				    const char *reason, struct hyhost_gc_acc *acc)
{
	...
	while (i < hc->line_pblocks && !out_of_lines) {
		...
		/* Collect phase: reserve up to HYHOST_GC_BATCH destination
		...
		while (i < hc->line_pblocks && n < READ_ONCE(hc->gc_batch) &&
		       scanned < HYHOST_GC_SCAN_CHUNK) {
			sector_t old_pblock = base_pblock + i;
			sector_t lpage      = hc->rmap[old_pblock];
			struct hyhost_mig_item *it;

			i++;
			scanned++;
			if (lpage == HYHOST_INVALID_LPA)
				continue;
			...
			if (!hc->gc_cur_line_active ||
				...
				if (hyhost_line_pop(hc, &new_line) < 0) {
					i--; /* slot not consumed */
					out_of_lines = true;
					break;
				}
		...
		if (n) {
			...
			hyhost_mig_wave(hc, n, REQ_OP_READ);
			read_wave_ns += ktime_to_ns(ktime_sub(ktime_get(), wv));
			wv = ktime_get();
			hyhost_mig_wave(hc, n, REQ_OP_WRITE);
			...
			for (j = 0; j < n; j++) {
				...
				if (hc->line_valid[new_line] == 0 &&
				    !hyhost_line_is_open(hc, new_line)) {
					hyhost_line_push_dirty(hc, new_line);
					atomic64_inc(&hc->stats.line_recycles);
					wake_erase = true;
				}
				if (it->failed)
	...
	if (migrated > 0) {
		...
		pr_info("dm-hyhost: [%s] GC line=%llu migrated_pages=%d gc_wall_ns=%lld read_wave_ns=%lld write_wave_ns=%lld\n",
...
/* Policy-driven GC: pick a victim via hyhost_select_victim (Greedy or
 * Cost-Benefit) and migrate its valid pblocks. Used by the background
 * GC kthread and the dmsetup "gc" message.
 */
static int hyhost_do_gc_one_line(struct hyhost_c *hc)
{
	sector_t victim;
	unsigned long flags;

	spin_lock_irqsave(&hc->lock, flags);
	victim = hyhost_select_victim(hc);
	 ...
	 * fully-valid lines). */
	if (victim != HYHOST_INVALID_PBA &&
	    hc->line_valid[victim] >= hc->line_pblocks / 100 * HYHOST_GC_VICTIM_MAX_VPC) {
		atomic64_inc(&hc->stats.gc_skipped);
		victim = HYHOST_INVALID_PBA;
	}
	if (victim == HYHOST_INVALID_PBA) {
		spin_unlock_irqrestore(&hc->lock, flags);
		return -1;
	...
	return hyhost_do_gc_one_line_at(hc, victim, "GC", NULL);
}
 ...
 * top down and migrates each non-empty line via hyhost_do_gc_one_line_at.
 *
...
static void hyhost_shrink_restore(struct hyhost_c *hc)
{
	unsigned long flags;
	size_t i;

	spin_lock_irqsave(&hc->lock, flags);
	hc->push_bound_lines = (hc->r_end >> HYHOST_PAGE_SHIFT) /
			       hc->line_pblocks;
	...
	if (hc->quar_count)
		pr_info("dm-hyhost: [SHRINK abort] restored %zu quarantined lines\n",
			hc->quar_count);
	hc->quar_count = 0;
	spin_unlock_irqrestore(&hc->lock, flags);
	wake_up(&hc->alloc_wait);
...
static int hyhost_prepare_shrink_force_gc(struct hyhost_c *hc,
					   sector_t new_r_pages)
{
	sector_t new_r_lines = new_r_pages / hc->line_pblocks;
	sector_t old_r_lines = (hc->r_end >> HYHOST_PAGE_SHIFT) /
	 ...
	 * boundary. After this, hyhost_line_pop() can only hand out
	...
	for (line = old_r_lines; line > new_r_lines; line--) {
		...
		atomic64_inc(&hc->stats.gc_runs);
		ret = hyhost_do_gc_one_line_at(hc, victim, "SHRINK", &acc);
		spin_lock_irqsave(&hc->lock, flags);
		v = hc->line_valid[victim];
		spin_unlock_irqrestore(&hc->lock, flags);
		if (ret < 0 || v != 0) {
	...
	pr_info("dm-hyhost: [SHRINK force-GC] TOTAL lines=%d migrated_pages=%lld (%lld MiB) event_ns=%lld gc_wall_ns=%lld read_ns=%lld write_ns=%lld range=[%llu..%llu)\n",
...
static int hyhost_gc_thread(void *data)
{
	struct hyhost_c *hc = data;
	...
	while (!kthread_should_stop()) {
		...
		while (!kthread_should_stop() &&
			...
			int n = hyhost_do_gc_one_line(hc);
			 ...
			 */
			if (n <= 0) {
				pr_info("dm-hyhost: [GC] parked (no useful victim, n=%d, free_lines=%zu)\n",
...
static int hyhost_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	struct hyhost_c *hc;
	...
	if (argc < 2 || argc > 3) {
		ti->error = "Usage: hyhost <dev> <r_end_lba> [max_r_end_lba]";
		return -EINVAL;
	}

	hc = kzalloc(sizeof(*hc), GFP_KERNEL);
	if (!hc) {
		ti->error = "Cannot allocate hyhost context";
		return -ENOMEM;
	}
	...
	if ((hc->r_end & HYHOST_PAGE_MASK) || (r_max_end & HYHOST_PAGE_MASK)) {
		ti->error = "r_end / max_r_end must be page-aligned (multiple of 8 sectors)";
		ret = -EINVAL;
		goto bad_arg;
	}

	hc->line_pblocks = HYHOST_LINE_PBLOCKS;
	hc->zone_pages   = HYHOST_ZONE_PAGES;
	...
	if (hc->r_max_pages) {
		...
		/* HYHOST_INVALID_PBA == ~0 → memset 0xff initialises the
		 ...
		 * the S-region. Grow (hyhost_quiesce_grow) pushes those lines on
	...
	hc->gc_policy = HYHOST_GC_GREEDY;
	hc->gc_batch        = HYHOST_GC_BATCH_DEFAULT;  /* drain knee */
	hc->gc_batch_shrink = HYHOST_GC_BATCH_SHRINK;   /* below the co-tenant cliff */
	 ...
	 * (see hyhost_set_watermarks). */
	hyhost_set_watermarks(hc, active_lines);
	...
	ti->per_io_data_size      = sizeof(struct hyhost_pio);
	ti->private               = hc;

	hc->erase_thread = kthread_run(hyhost_erase_thread, hc,
				       "hyhost-erase/%s", hc->dev->name);
	...
	hc->gc_thread = kthread_run(hyhost_gc_thread, hc, "hyhost-gc/%s",
				    hc->dev->name);
	...
	hc->zmod_wq = alloc_ordered_workqueue("hyhost-zmod/%s",
...
static void hyhost_dtr(struct dm_target *ti)
{
	struct hyhost_c *hc = ti->private;
...
static int hyhost_map_r_page(struct hyhost_c *hc, struct bio *bio,
			     sector_t bio_lba)
{
	sector_t lpage = bio_lba >> HYHOST_PAGE_SHIFT;
	sector_t pblock_lba;
...
retry_alloc:
		...
		if (!hc->cur_line_active ||
			...
			if (hc->ring_count > hc->gc_reserve_lines &&
			    hyhost_line_pop(hc, &hc->cur_line_idx) == 0) {
				hc->cur_line_off    = 0;
				hc->cur_line_active = true;
			} else if (hc->l2p[lpage] != HYHOST_INVALID_PBA) {
		...
		if (stall) {
			...
			if (stalled && READ_ONCE(hc->gc_blocked) &&
			    (READ_ONCE(hc->r_end) >> HYHOST_PAGE_SHIFT) >= hc->r_max_pages) {
			 ...
			 * is honored immediately. */
			{
				long wr = wait_event_killable_timeout(hc->alloc_wait,
					   ...
					   READ_ONCE(hc->ring_count) >
						   hc->gc_reserve_lines ||
					   (READ_ONCE(hc->gc_blocked) &&
					    (READ_ONCE(hc->r_end) >> HYHOST_PAGE_SHIFT) >=
							hc->r_max_pages),
					   60 * HZ);
				if (wr <= 0) {
					atomic64_inc(&hc->stats.r_nospc);
		...
		if (inplace) {
			...
			pio = dm_per_bio_data(bio, sizeof(struct hyhost_pio));
			pio->op         = HYHOST_OP_R_WRITE;
			pio->lpage      = lpage;
...
static int hyhost_map_r_write(struct hyhost_c *hc, struct bio *bio,
			      sector_t bio_lba, unsigned int n)
{
	sector_t lpage  = bio_lba >> HYHOST_PAGE_SHIFT;
	unsigned int npages = n >> HYHOST_PAGE_SHIFT;
	unsigned int k = 0;
	...
	if (k == 0) {
		if (n > HYHOST_PAGE_SECTORS)
			dm_accept_partial_bio(bio, HYHOST_PAGE_SECTORS);
		return hyhost_map_r_page(hc, bio, bio_lba);
	}
	...
	if (k < npages) {
		dm_accept_partial_bio(bio, (unsigned int)(k << HYHOST_PAGE_SHIFT));
	...
	bio->bi_iter.bi_sector = pblock_start << HYHOST_PAGE_SHIFT;
	return DM_MAPIO_REMAPPED;
}
 ...
 */
static void hyhost_zmod_worker(struct work_struct *work)
{
	struct hyhost_zmod_ctx *ctx =
		container_of(work, struct hyhost_zmod_ctx, work);
	struct hyhost_c *hc = ctx->hc;
	...
	if (is_shrink) {
		ret = hyhost_prepare_shrink_force_gc(hc, ctx->new_r_pages);
		if (ret) {
	...
	spin_lock_irqsave(&hc->lock, flags);
	if (is_shrink)
		hyhost_quiesce_shrink(hc, ctx->new_r_pages);
	else if (ctx->new_r_end_lba > hc->r_end)
		hyhost_quiesce_grow(hc, ctx->new_r_pages);
	WRITE_ONCE(hc->r_end, ctx->new_r_end_lba);
	hyhost_set_watermarks(hc, ctx->new_r_pages / hc->line_pblocks); /* scale GC band to new R */
	spin_unlock_irqrestore(&hc->lock, flags);
	/* A grow pushed reclaimed lines onto the free ring. */
	wake_up(&hc->alloc_wait);
	z_q = ktime_get();

	pr_info("dm-hyhost: [ZMOD %s] forcegc=%lld us device_submit(ZSA)=%lld us quiesce=%lld us total=%lld us\n",
...
static int hyhost_map(struct dm_target *ti, struct bio *bio)
{
	struct hyhost_c *hc = ti->private;
	struct hyhost_pio *pio;
	...
	if (bio_op(bio) == REQ_OP_ZONE_MODIFY) {
		...
		if (new_r_end_lba & HYHOST_PAGE_MASK) {
			bio->bi_status = BLK_STS_NOTSUPP;
			bio_endio(bio);
			return DM_MAPIO_SUBMITTED;
		}
		new_r_pages = new_r_end_lba >> HYHOST_PAGE_SHIFT;
		/* r_end moves zone-granular (1 GiB), matching FEMU's hyhost_set_r_end
		...
		INIT_WORK(&ctx->work, hyhost_zmod_worker);
	...
	if (bio_op(bio) == REQ_OP_DISCARD) {
		...
		hyhost_discard_range(hc, bio_lba, n, r_end);
		bio->bi_status = BLK_STS_OK;
		bio_endio(bio);
		return DM_MAPIO_SUBMITTED;
	}
	...
	if ((bio_lba & HYHOST_PAGE_MASK) || (n & HYHOST_PAGE_MASK)) {
		atomic64_inc(&hc->stats.r_align_rejects);
		bio->bi_status = BLK_STS_NOTSUPP;
		bio_endio(bio);
		return DM_MAPIO_SUBMITTED;
	...
	if (r_coalesce && op_is_write(bio_op(bio)))
		return hyhost_map_r_write(hc, bio, bio_lba, n);
 ...
 */
static int hyhost_end_io(struct dm_target *ti, struct bio *bio,
			 blk_status_t *error)
{
	struct hyhost_c   *hc  = ti->private;
	struct hyhost_pio *pio = dm_per_bio_data(bio, sizeof(*pio));
	sector_t lpage, new_pblock, new_line;
	unsigned int npages, i;

	if (pio->op != HYHOST_OP_R_WRITE)
	...
	if (*error == BLK_STS_OK) {
		...
		for (i = 0; i < npages; i++) {
			...
			if (old != HYHOST_INVALID_PBA) {
				old_line = old / hc->line_pblocks;
				BUG_ON(hc->line_valid[old_line] == 0);
				hc->line_valid[old_line]--;
				hc->rmap[old] = HYHOST_INVALID_LPA;
				if (hc->line_valid[old_line] == 0 &&
				    !hyhost_line_is_open(hc, old_line)) {
					hyhost_line_push_dirty(hc, old_line);
					atomic64_inc(&hc->stats.line_recycles);
					recycled = true;
				}
...
static int hyhost_iterate_devices(struct dm_target *ti,
 ...
 * page-aligned), and hyhost_discard_range()'s inward rounding then leaks
 * one untrimmed page per split boundary. Page-sized granularity makes
 * every split page-aligned. */
static void hyhost_io_hints(struct dm_target *ti, struct queue_limits *limits)
{
	limits->discard_granularity = HYHOST_PAGE_BYTES;
	limits->max_discard_sectors = UINT_MAX & ~HYHOST_PAGE_MASK;
	limits->max_hw_discard_sectors = UINT_MAX & ~HYHOST_PAGE_MASK;
}

static int hyhost_prepare_ioctl(struct dm_target *ti, struct block_device **bdev)
{
	struct hyhost_c *hc = ti->private;
...
static int hyhost_report_zones(struct dm_target *ti,
...
#else
#define hyhost_report_zones NULL
#endif

static void hyhost_status(struct dm_target *ti, status_type_t type,
			  ...
			  unsigned int maxlen)
{
	...
	case STATUSTYPE_INFO: {
		...
		DMEMIT("r_end=%llu pages=%llu lines=%llu line_pblocks=%u zone_pblocks=%u block_pages=%u gc_policy=%s free_lines=%llu dirty=%llu cur_off=%u r_bios=%llu s_bios=%llu splits=%llu w=%llu r=%llu unmapped=%llu ovr=%llu inplace=%llu recycles=%llu gc_runs=%llu gc_mig=%llu gc_skip=%llu erases=%llu erase_fail=%llu requeue=%llu nospc=%llu wfail=%llu discards=%llu discard_pgs=%llu valid_pages=%llu valid_lines=%llu vh=%u:%u:%u:%u:%u best_victim_vpc=%u free_pages=%llu gc_blocked=%u",
		       ...
		       hc->zone_pages,
		       HYHOST_PGS_PER_BLK,
		       hc->gc_policy == HYHOST_GC_COST_BENEFIT ? "cb" : "greedy",
 ...
 */
static int hyhost_message(struct dm_target *ti, unsigned int argc, char **argv,
			  char *result, unsigned int maxlen)
{
	...
	if (!strcmp(argv[0], "set_r_end")) {
		...
		if (argc != 2 || sscanf(argv[1], "%llu%c", &val, &dummy) != 1)
			return -EINVAL;
		if (val & HYHOST_PAGE_MASK) {
			DMWARN("set_r_end %llu not page-aligned", val);
			return -EINVAL;
		}
		new_r_pages = (sector_t)val >> HYHOST_PAGE_SHIFT;
		/* Zone-granular (1 GiB), matching FEMU's hyhost_set_r_end. */
		if (new_r_pages % hc->zone_pages) {
		...
		if (is_shrink) {
			ret = hyhost_prepare_shrink_force_gc(hc, new_r_pages);
			if (ret) {
		...
		/* Device accepted the new boundary. Commit dm-side state. */
		{
			...
			if ((sector_t)val < cur_r_end)
				hyhost_quiesce_shrink(hc, new_r_pages);
			else if ((sector_t)val > cur_r_end)
				hyhost_quiesce_grow(hc, new_r_pages);
			WRITE_ONCE(hc->r_end, (sector_t)val);
			hyhost_set_watermarks(hc, new_r_pages / hc->line_pblocks); /* scale GC band to new R */
			spin_unlock_irqrestore(&hc->lock, flags);
			/* A grow pushed reclaimed lines onto the free ring. */
			wake_up(&hc->alloc_wait);
		}
	...
	if (!strcmp(argv[0], "gc_policy")) {
		...
		if (!strcmp(argv[1], "greedy")) {
			WRITE_ONCE(hc->gc_policy, HYHOST_GC_GREEDY);
		} else if (!strcmp(argv[1], "cb")) {
			WRITE_ONCE(hc->gc_policy, HYHOST_GC_COST_BENEFIT);
		} else {
	...
	if (!strcmp(argv[0], "gc_batch")) {
		/* Runtime GC migration batch cap (1..HYHOST_GC_BATCH). Smaller =>
		...
		if (argc != 2 || kstrtou32(argv[1], 10, &v))
			return -EINVAL;
		if (v < 1) v = 1;
		if (v > HYHOST_GC_BATCH) v = HYHOST_GC_BATCH;
...
static struct target_type hyhost_target = {
	.name            = "hyhost",
	.version         = {0, 15, 0},
	...
	.dtr             = hyhost_dtr,
	.map             = hyhost_map,
	.end_io          = hyhost_end_io,
	.status          = hyhost_status,
	.message         = hyhost_message,
	.prepare_ioctl   = hyhost_prepare_ioctl,
	.iterate_devices = hyhost_iterate_devices,
	.io_hints        = hyhost_io_hints,
	.report_zones    = hyhost_report_zones,
};

static int __init dm_hyhost_init(void)
{
	int r = dm_register_target(&hyhost_target);
...
static void __exit dm_hyhost_exit(void)
{
	dm_unregister_target(&hyhost_target);
}

module_init(dm_hyhost_init);
module_exit(dm_hyhost_exit);

MODULE_AUTHOR("HYSSD project");
MODULE_DESCRIPTION("dm target for the HYHOSTSSD host-managed hybrid FTL");
MODULE_LICENSE("GPL");
