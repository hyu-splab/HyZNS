#!/bin/bash
#
# Single-mount F2FS on dm-hyhost (R+S hybrid, like HYSSD).
#
...
if (( R_END % HYHOST_ZONE_SECTORS != 0 )); then
    echo "ERROR: R_END_LBA must be zone-aligned (multiple of ${HYHOST_ZONE_SECTORS} sectors = 1 GiB)" >&2
    exit 1
fi
...
NUM_RZONE=$((R_END / HYHOST_ZONE_SECTORS))
...
echo "0 ${SIZE_SECTORS} hyhost ${BACKING} ${R_END}" | dmsetup create "${NAME}"
