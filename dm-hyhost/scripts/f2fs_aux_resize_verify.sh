...
echo "=== setup: dm-hyhost target, device boundary -> ${R_INIT} GiB R ==="
umount "$MNT" 2>/dev/null || true
dmsetup remove "$NAME" 2>/dev/null || true
lsmod | awk '{print $1}' | grep -qx dm_hyhost || ./scripts/load.sh >/dev/null
...
echo "0 $SZ hyhost $BACKING $((R_INIT*ZS)) $SZ" | dmsetup create "$NAME"
