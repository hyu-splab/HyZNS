#!/bin/bash
#
# dev_bw.sh - raw bandwidth matrix for the FEMU HYHOSTSSD (geometry via GEOM env).
# Measures the device ceilings that bound mixed workloads and compares them with
# the FEMU latency model (hyhost.c).
#
...
ZS=$HYHOST_ZONE_SECTORS                                   # zone sectors (1GiB)
...
echo "0 $sz hyhost $DEV $((R_ZONES*ZS)) $sz" | dmsetup create "$DM" || die "dmsetup create"
