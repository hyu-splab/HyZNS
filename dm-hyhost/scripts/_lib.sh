#!/bin/bash
#
# Shared helpers for the dm-hyhost scripts. Source it, don't run it:
#   source "$(dirname "$0")/_lib.sh"
#
# Conventions assumed by callers:
#   - BACKING defaults to /dev/nvme0n1 (the FEMU HYHOSTSSD namespace).
#   - 1 GiB zones (= one dm-hyhost line), 512B sectors.

HYHOST_ZONE_SECTORS=$(( 1024 * 1024 * 1024 / 512 ))   # 1 GiB / 512B
...
# /dev/mapper/hyhost* target (dm-hyhost answers it locally from its r_end).
report_rzones() {
    python3 - "$1" <<'PY'
...
reset_device_full_r() {
    ...
    total_zones=$(( size_sectors / HYHOST_ZONE_SECTORS ))
    nr_rz="$(report_rzones "$backing")"

    if (( nr_rz < total_zones )); then
        s_start=$(( nr_rz * HYHOST_ZONE_SECTORS ))
