#!/bin/bash
# gc_lat_hyhostr.sh - measure dm-hyhost host-FTL line-GC relocation latency.
# Create garbage in R (fill + random-overwrite -> partial-valid lines), force a
# GC sweep, read per-line "dm-hyhost: GC line=.. migrated_pages=.. gc_wall_ns=.."
...
set -uo pipefail
cd "$(dirname "$0")/.."
source scripts/_lib.sh; ZS=$HYHOST_ZONE_SECTORS
BACK=/dev/nvme0n1; NAME=hyhost0; DEV=/dev/mapper/$NAME
RZ=${RZ:-4}; FILL=${FILL:-3G}; OW=${OW:-2G}
OUT=/data/HYSSD/hyhostd/results/gc_dev_compare; mkdir -p "$OUT"; STAMP=$(date +%Y%m%d_%H%M%S); RES=$OUT/hyhostr_gc_$STAMP.txt
...
lsmod|awk '{print $1}'|grep -qx dm_hyhost || ./scripts/load.sh >/dev/null 2>&1
reset_device_full_r "$BACK" >/dev/null 2>&1||true
echo "0 $((RZ*ZS)) hyhost $BACK $((RZ*ZS))" | dmsetup create "$NAME" || { echo "dm create fail"|tee $RES; exit 1; }
dmsetup message "$NAME" 0 set_r_end $((RZ*ZS))
echo "hyhostr_gc $STAMP: R=$RZ zones, fill=$FILL ow=$OW (FEMU R 450us media)" | tee $RES
...
echo "=== per-line GC log (dm-hyhost) ===" | tee -a $RES
dmesg | grep "dm-hyhost: GC line" | tee -a $RES
echo "=== throughput per line (migrated*4KiB / gc_wall_ns) ===" | tee -a $RES
dmesg | grep "dm-hyhost: GC line" | awk '{
