...
/**
 * zbd_get_reserved_zones - Get reserved zone for meta information
 */
static int zbd_get_reserved_zones(int fd, char *devname, struct zbd_info *zbdi) {
	unsigned int nr_zones = 0;
	int ret;
	ret = ioctl(fd, BLKREPORTABA, &nr_zones);
	fprintf(stdout, "reserved zones: %u, ret: %d\n", nr_zones, ret);
	if (ret < 0) {
		fprintf(stdout, "%s: not supporting BLKREPORTABA(maybe no HYSSD), ret: %d\n", devname, ret);
		nr_zones = 0;
	}
	
	zbdi->nr_rzones = nr_zones;
	return 0;
}
...
static struct zbd_info *zbd_do_get_info(int fd, char *devname)
{
	...
	ret = zbd_get_reserved_zones(fd, devname, zbdi);
	if (ret)
		goto err;
