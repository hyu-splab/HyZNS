 ...
 * dm-hyzns) and the ZNS area (S, from the zone-aware FS or `nvme report-zones`)
 ...
 * r_end granularity) is 16 dm-hyzns lines (a line is the 64-MiB GC unit), so
 * zone sectors = zone_pblocks*8, NOT line_pblocks*8. dm-hyzns emits both
...
#define HYZNSD_VERSION "0.1"

/* ---- contract: dm-hyzns status keys (dmsetup status <dm>) ---------------- */
...
#define HYZNS_PAGE_SECTORS 8u             /* 4 KiB page in 512B sectors       */
...
typedef struct {
	char        dm[64];          /* dm-hyzns target name (e.g. hyzns0)     */
