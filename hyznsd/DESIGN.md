...
(from `dm-hyzns`) and the ZNS area (from the zone-aware FS, with an `nvme
...
## Roles (ownership)
- **dm-hyzns** owns the CNS page-FTL → reports **R_C** (free CNS zones) via `dmsetup status` (`free_lines`), current ABA via `r_end`.
...
- **hyznsd** owns the **policy** (when/how much), backoff, hysteresis, logging.

## Units
`free_lines`, `r_end` are in the dm-hyzns line/sector space. 1 line = 1 zone = `S_Z`.
- `current_R` (zones) = `r_end / HYZNS_ZONE_SECTORS`
...
## Config (per device)
`dm=hyzns0 backing=/dev/nvme0n1 fs=f2fs|zenfs mnt=/mnt/aux aux=/mnt/aux
