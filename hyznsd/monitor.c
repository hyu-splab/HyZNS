/* monitor.c - read live R_C (dm-hyzns) and R_Z (FS | report-zones). */
#define _GNU_SOURCE
#include "hyznsd.h"
#include <stdio.h>
#include <stdlib.h>
...
/* ------------------------------------------------------------------------- *
 * R_C: dm-hyzns via `dmsetup status <dm>`
 *
 *   0 <len> hyzns ... r_end=<sec> line_pblocks=<p> zone_pblocks=<z> ... free_pages=<n> ... nospc=<n>
 *
...
int hyd_read_dm(const hyd_cfg *cfg, hyd_state *st)
{
	...
	/* dm target length (2nd token: "0 <len> hyzns ..."): the S-region ends
	...
	uint32_t zone_sectors = (uint32_t)zone_pblk * HYZNS_PAGE_SECTORS;
