#!/bin/bash
#
# Run FEMU as a Host-managed Hybrid SSD (HYHOSTSSD, femu_mode=5).
#
# Mirrors run-hyssd.sh for the VM/host plumbing (KVM, multi-disk, qmp,
...
# Device sizing. Match run-hyssd.sh's 128 GiB default; tweak as needed.
ssd_size_hh=262144              # 16G=16384 / 64G=65536 / 96G = 98304 / 128G=131072 / 256G=262144

# HYHOSTSSD NAND geometry. line = nchs*luns_per_ch*pgs_per_blk*page = the
...
# NOTE: nchs/luns_per_ch must equal dm-hyhost's HYHOST_NCHS/HYHOST_LUNS_PER_CH.
...
# HYHOST_PGS_PER_BLK*4 KiB, or the (ch,lun) timing model desyncs.
hh_nchs=8
hh_luns_per_ch=4
hh_page_bytes=16384
hh_pgs_per_blk=128
...
FEMU_OPTIONS_HH=${FEMU_OPTIONS_HH}",hyhost_nchs=${hh_nchs}"
FEMU_OPTIONS_HH=${FEMU_OPTIONS_HH}",hyhost_luns_per_ch=${hh_luns_per_ch}"
FEMU_OPTIONS_HH=${FEMU_OPTIONS_HH}",hyhost_page_bytes=${hh_page_bytes}"
FEMU_OPTIONS_HH=${FEMU_OPTIONS_HH}",hyhost_pgs_per_blk=${hh_pgs_per_blk}"
FEMU_OPTIONS_HH=${FEMU_OPTIONS_HH}",hyhost_zone_size_mb=${hh_zone_size_mb}"
...
sudo ${FEMU_LAUNCH} \
    -name "FEMU-HYHOSTSSD-VM",debug-threads=on \
    -enable-kvm \
    -cpu host \
    -smp ${FEMU_SMP} \
    -m 32G \
    ...
    -nographic \
    -qmp unix:./qmp-sock,server,nowait 2>&1 | tee hyhost_debug.log
