...
# identical media (validation intent, matches run-hyzns-dual.sh).
#
# Counterpart of run-hyzns-dual.sh (which does hyzns-multi = HYZNS+BBSSD).
...
#   ch/lun matched to HYZNS (32x16); ConfZNS 450/80/2000 us latency.
#
...
FEMU_OPTIONS_ZNS=${FEMU_OPTIONS_ZNS}",secs_per_pg=32"      # 16 KiB page (== HYZNS)
FEMU_OPTIONS_ZNS=${FEMU_OPTIONS_ZNS}",pgs_per_blk=128"     # 2 MiB block (== HYZNS)
FEMU_OPTIONS_ZNS=${FEMU_OPTIONS_ZNS}",luns_per_ch=16"      # == HYZNS
FEMU_OPTIONS_ZNS=${FEMU_OPTIONS_ZNS}",nchs=32"             # == HYZNS
...
#   Geometry/timing matched to the ZNS device above (== HYZNS R-region),
#   so aux media == ZNS media.
#   OVERPROVISIONING (see run-hyzns-dual.sh for the full rationale): physical
