...
#
#   /dev/nvme0n1 = HYHOSTSSD (femu_mode=5, 256 GiB)  -- host-managed hybrid FTL
...
# dm-hyhost host-FTL (out-of-place + GC) overhead or just FEMU's NAND timing.
# To make that comparison valid, the BBSSD's NAND timing AND geometry are set
# IDENTICAL to the HYHOSTSSD R-region (same ch x lun, page size, and per-op
# latencies). Any throughput delta is then host-FTL vs device-FTL, not media.
#
# HYHOSTSSD R-region geometry/timing baked in hyhostssd/hyhost.c:
...
# ---------------------------------------------------------------------------
# Device 0: HYHOSTSSD (femu_mode=5)  ->  /dev/nvme0n1
# ---------------------------------------------------------------------------
...
#   Geometry/timing matched to HYHOSTSSD R-region (see header).
...
secs_per_pg=32         # 16 KiB page  (== HYHOSTSSD HYHOST_DEFAULT_PAGE_BYTES)
pgs_per_blk=128        # 2 MiB erase block  (== HYHOST_DEFAULT_PGS_PER_BLK)
blks_per_pl=16         # -> 16 GiB PHYSICAL at the geometry above (NOT logical)
pls_per_lun=1          # no multiplane (HYHOSTSSD has none)
luns_per_ch=16         # == HYHOST_DEFAULT_LUNS_PER_CH
nchs=32               # == HYHOST_DEFAULT_NCHS

pg_rd_lat=80000        # == HYHOST_PG_RD_LAT_NS
pg_wr_lat=450000       # == HYHOST_PG_WR_LAT_NS
blk_er_lat=2000000     # == HYHOST_BLK_ER_LAT_NS
ch_xfer_lat=25000      # == HYHOST_CH_XFER_LAT_NS  (BBSSD default is 0; matched on purpose)
...
sudo ${FEMU_LAUNCH} \
    -name "FEMU-HYHOSTSSD-DUAL-VM",debug-threads=on \
    -enable-kvm \
    -cpu host \
    -smp ${FEMU_SMP} \
    -m 32G \
    ...
    -nographic \
    -qmp unix:./qmp-sock,server,nowait 2>&1 | tee hyhost_dual_debug.log
