...
typedef struct {
    ...
    bb_line_info lines[]; /* Changed to C99 flexible array member syntax */
} bb_line_mgmt_info;

typedef struct {
    bb_write_pointer_info wp_info;
    uint64_t gc_count;      /* Number of GC operations performed */
    uint64_t gc_pgs;        /* # of pages moved in GC */
    bb_line_mgmt_info lm_info;
} bb_monitor_data;
...
#define DE_err(fmt, ...) \
    do { \
        ...
        fprintf(stderr, "[DE-stat] [ %s ]: " fmt, timestamp, ## __VA_ARGS__); \
    } while (0)
