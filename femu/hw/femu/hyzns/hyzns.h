...
/*
 * Host-managed Hybrid SSD (HYZNS) — femu_mode = 5.
 *
 ...
 * HYZNS_ZA_SET_R_END (0x20). The new r_end LBA arrives in the standard
 * SLBA field (cdw10/cdw11), matching every other ZSA's wire encoding.
 * Issued by dm-hyzns through REQ_OP_ZONE_MODIFY (bi_sector = new ABA LBA)
 ...
 *
 * Wire-level zone-type convention:
 *   Every zone descriptor reports d.zt = HYZNS_ZONE_TYPE_SEQ_WRITE (0x02)
 ...
 *     - internal: HyZNSZone::is_random tags R-region zones (slba < r_end).
 *       hyzns_classify_zones flips this on init and on every set_r_end.
 ...
 *       FTL's job (dm-hyzns L2P + the EMPTY-only invariant on set_r_end).
 *     - host learns R-zone count via Zone Management Receive action
 *       HYZNS_ZRA_REPORT_RZONE (0x21), exposed to userspace through the
...
/* -------------------------------------------------------------------------
 * Zone primitives (independent copy; zns/ declares its own).
 * Layout matches the NVMe ZNS spec so libzbd / kernel zone code work.
 * ------------------------------------------------------------------------- */

enum HyZNSZoneType {
    HYZNS_ZONE_TYPE_SEQ_WRITE  = 0x02, /* only wire-level type — see header comment */
};

enum HyZNSZoneState {
    HYZNS_ZS_RESERVED          = 0x00,
    HYZNS_ZS_EMPTY             = 0x01,
    HYZNS_ZS_IMPLICITLY_OPEN   = 0x02,
    HYZNS_ZS_EXPLICITLY_OPEN   = 0x03,
    HYZNS_ZS_CLOSED            = 0x04,
    HYZNS_ZS_READ_ONLY         = 0x0D,
    HYZNS_ZS_FULL              = 0x0E,
    HYZNS_ZS_OFFLINE           = 0x0F,
};

enum HyZNSZoneSendAction {
    HYZNS_ZA_CLOSE        = 0x01,
    HYZNS_ZA_FINISH       = 0x02,
    HYZNS_ZA_OPEN         = 0x03,
    HYZNS_ZA_RESET        = 0x04,
    HYZNS_ZA_OFFLINE      = 0x05,
    HYZNS_ZA_SET_R_END    = 0x20, /* device-scoped; new r_end LBA in SLBA */
    HYZNS_ZA_R_BLOCK_ERASE = 0x22, /* R-region only; slba = R-block start (debug/legacy) */
    HYZNS_ZA_R_LINE_ERASE  = 0x23, /* R-region only; slba = line start, parallel erase across all (ch, lun) */
};

enum HyZNSZoneReceiveAction {
    HYZNS_ZRA_REPORT       = 0x00,
    HYZNS_ZRA_REPORT_RZONE = 0x21, /* vendor: returns u32 R-zone count via PRP */
};

typedef struct QEMU_PACKED HyZNSZoneDescr {
    uint8_t  zt;
...
typedef struct QEMU_PACKED HyZNSZoneReportHeader {
    uint64_t nr_zones;
    uint8_t  rsvd[56];
} HyZNSZoneReportHeader;
 ...
 * (same convention as zns/). The struct names match the forward
...
typedef struct HyZNSSSD {
     ...
     * erase unit and the host FTL (dm-hyzns) GC unit. line_size (in LBAs) =
     * nchs * luns_per_ch * pgs_per_blk * (page_bytes / lba_bytes). A zone is
     * an integer number of lines (zone_size % line_size == 0). Used to
     * validate HYZNS_ZA_R_LINE_ERASE alignment — dm issues one erase per
