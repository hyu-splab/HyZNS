...
static void nvme_process_sq_io(void *opaque, int index_poller)
{
    ...
    while (!(nvme_sq_empty(sq))) {
        /* Defensive: the req pool is finite (sq->size reqs). If it is
         * momentarily drained (all reqs in flight through to_ftl/to_poller),
         * stop pulling commands rather than dequeue a garbage entry off the
         * empty list; resume next poll once the CQ poller returns reqs.
         * Checked before consuming the SQ entry so head/cmd stay untouched. */
        if (QTAILQ_EMPTY(&sq->req_list)) {
            break;
        }
        ...
        if (status == NVME_SUCCESS) {
            ...
            /* Failed command: still hand it to the FTL ring so the poller
             * posts a CQE with the non-zero status back to the guest.
             * Without this, the guest hangs waiting for completion. */
            int rc = femu_ring_enqueue(n->to_ftl[index_poller], (void *)&req, 1);
            if (rc != 1) {
                femu_err("enqueue (err path) failed, ret=%d\n", rc);
            }
        }
...
static void nvme_process_cq_cpl(void *arg, int index_poller)
{
    ...
    while ((req = pqueue_peek(pq))) {
        ...
        /* Pop first (matches upstream FEMU). The previous code popped only on
         * the cq-active path and `continue`d otherwise, which left the req in
         * the pqueue forever (infinite re-peek) AND never returned it to
         * sq->req_list — draining the request pool. Once the pool was empty,
         * nvme_process_sq_io pulled a garbage req off the empty req_list and
         * the CQ poller faulted on req->expire_time (dual+BBSSD crash). */
        pqueue_pop(pq);

        cq = n->cq[req->sq->sqid];
        if (!cq->is_active) {
            /* CQ torn down: still return the req to the pool, don't leak it. */
...
void *nvme_poller(void *arg)
{
    ...
    case 1:
        while (1) {
            ...
            /* Quiesce handshake (Dekker-style): mark in_sweep, full barrier,
             * then re-check dataplane_started. If nvme_clear_ctrl() cleared it
             * after the top check but before this barrier, we observe it here
             * and back off, so we never touch queue/req memory it is about to
             * free. Conversely clear_ctrl, after clearing dataplane_started,
             * waits for in_sweep==false, so it never frees under a live sweep.
             * (Follows the upstream MoatLab/FEMU fix.) */
            ...
            if (sq && sq->is_active && cq && cq->is_active) {
                /* acquire: pair with smp_wmb() before is_active=true in
                 * nvme_create_sq/cq so we see a fully-initialized sq/cq. */
                smp_rmb();
                nvme_process_sq_io(sq, index);
            }
            nvme_process_cq_cpl(n, index);
            /* release: this sweep's queue writes must be globally visible
             * before clear_ctrl can observe in_sweep==false and free. */
            smp_mb();
            n->poller_in_sweep[index] = false;
        }
        break;
...
static uint16_t nvme_io_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    case NVME_CMD_DSM:
        ...
        /* Modes with their own FTL (BBSSD bb_discard, HYHOSTSSD no-op, ...)
         * must handle DSM themselves. The previous code called the generic
         * nvme_dsm() here and then FELL THROUGH (no return) into the default
         * case, invoking ext_ops.io_cmd as well — every DSM ran twice. For
         * BBSSD that meant the generic path AND bb_discard; the generic
         * nvme_dsm also only DMAs one DSM range (sizeof(*range), not nr) and
         * mutates req/status mid-flight, corrupting the request that then went
         * to the FTL ring -> CQ poller faulted on a bad req (dual crash). */
        if (n->ext_ops.io_cmd) {
