...
static void nvme_clear_ctrl(FemuCtrl *n, bool shutdown)
{
    ...
    /* Quiesce the pollers before freeing queues below. Without this, a poller
     * mid-sweep dereferences a freed req / dbbuf shadow (use-after-free on
     * controller reset / guest reboot). Pairs with the Dekker handshake in
     * nvme_poller(): dataplane_started is now false + barrier, so any poller
     * observes it and clears its in_sweep flag; wait here until all are idle.
     * (Upstream MoatLab/FEMU fix.) */
    smp_mb();
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_UINT32("zns_num_ch", FemuCtrl, zns_params.num_ch, 2), // ZNS params
    DEFINE_PROP_UINT32("zns_num_lun", FemuCtrl, zns_params.num_lun, 4),
    DEFINE_PROP_UINT64("zns_read", FemuCtrl, zns_params.pg_rd_lat, 40000), // ZNS Latency
    DEFINE_PROP_UINT64("zns_write", FemuCtrl, zns_params.pg_wr_lat, 200000),
    DEFINE_PROP_UINT64("zns_erase", FemuCtrl, zns_params.blk_er_lat, 2000000),
    DEFINE_PROP_UINT64("zns_ch_xfer", FemuCtrl, zns_params.ch_xfer_lat, 2000000),
    ...
    DEFINE_PROP_INT32("blks_per_pl", FemuCtrl, bb_params.blks_per_pl, 256), // == # of lines
    DEFINE_PROP_INT32("pls_per_lun", FemuCtrl, bb_params.pls_per_lun, 1),
    DEFINE_PROP_INT32("luns_per_ch", FemuCtrl, bb_params.luns_per_ch, 16),
    DEFINE_PROP_INT32("nchs", FemuCtrl, bb_params.nchs, 32),
    DEFINE_PROP_INT32("chs_per_line", FemuCtrl, bb_params.chs_per_line, 16), /* default: all channels */
    DEFINE_PROP_INT32("luns_per_line", FemuCtrl, bb_params.luns_per_line, 4), /* default: all LUNs */
    ...
    DEFINE_PROP_INT32("num_rzones", FemuCtrl, sp.num_rzones, 1),
    DEFINE_PROP_INT32("max_rzones", FemuCtrl, sp.max_rzones, 0), /* 0 = num_zones */
    DEFINE_PROP_INT32("num_op_line", FemuCtrl, sp.num_op_line, 0), // not used
    DEFINE_PROP_INT32("set_time_model", FemuCtrl, sp.set_time_model, 0), //xfer_time_model
    /* HYHOSTSSD (femu_mode=5) geometry. Defaults: 8ch x 4way x 128 pg/blk x
     * 16 KiB NAND page => block = 2 MiB, line = 8*4*128*16KiB = 64 MiB
     * (device erase / host GC unit). zone_size_mb=1024 keeps a 1 GiB logical
     * zone = 16 lines. The host LBA stays 4 KiB (NVMe LBA format); the 16 KiB
     * NAND page is the striping/latency unit only. */
    DEFINE_PROP_INT32("hyhost_nchs", FemuCtrl, hyhost_params.nchs, 8),
    DEFINE_PROP_INT32("hyhost_luns_per_ch", FemuCtrl, hyhost_params.luns_per_ch, 4),
    DEFINE_PROP_INT32("hyhost_page_bytes", FemuCtrl, hyhost_params.page_bytes, 16384),
    DEFINE_PROP_INT32("hyhost_pgs_per_blk", FemuCtrl, hyhost_params.pgs_per_blk, 128),
    DEFINE_PROP_INT32("hyhost_zone_size_mb", FemuCtrl, hyhost_params.zone_size_mb, 1024),
