...
static uint16_t nvme_create_sq(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    /* release: publish the fully-initialized sq before the poller can observe
     * is_active=true (pairs with smp_rmb() in nvme_poller). */
    smp_wmb();
    sq->is_active = true;
...
static void nvme_init_poller(FemuCtrl *n)
{
    ...
    for (i = 1; i <= n->nr_pollers; i++) {
        args[i].n = n;
        args[i].index = i;
        /* Name must be <=15 chars: Linux TASK_COMM_LEN is 16 incl. NUL, so
         * pthread_setname_np() ERANGE-fails on longer names and the thread
         * silently keeps its parent vCPU's comm ("CPU N/KVM"), which breaks
         * per-thread pinning (pin-threads.sh matches "femu-poller*"). */
