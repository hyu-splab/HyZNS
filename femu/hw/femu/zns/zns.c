...
static int zns_init_zone_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    n->zone_size = zone_size / lbasz; // 128MB / 512B = 262144
    n->zone_capacity = zone_cap / lbasz; // 128MB / 512B = 262144
    n->num_zones = ns->size / lbasz / n->zone_size; // 60GB / 512 / 262144 = 480
...
static uint64_t zns_advance_status(struct zns *ssd, struct ppa *ppa, struct nand_cmd *ncmd)
{
    int c = ncmd->cmd;
...
static uint16_t zns_zone_mgmt_send(FemuCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_ZONE_ACTION_FINISH:
        ...
        for (lpn = slpn; lpn <= elpn; lpn++) {
            ...
            curlat = zns_advance_status(zns, &ppa, &swr);
    ...
    case NVME_ZONE_ACTION_RESET:
        ...
        *resets = 1;
        // uint64_t maxlat = zns_advance_status(n, ns, cmd, req);
        uint64_t maxlat = 0, curlat = 0;
        struct zns *zns = n->zns;
        struct ppa ppa;

        for (int i = 0; i < zns->sp.nchs; i++) {
            for (int j = 0; j < zns->sp.luns_per_ch; j++) {
                ppa.ppa = 0;
                ppa.g.ch = i;
                ppa.g.lun = j;
                ...
                curlat = zns_advance_status(zns, &ppa, &srd);
                maxlat = (curlat > maxlat) ? curlat : maxlat;
            }
        }
        req->reqlat = maxlat;
                         ...
                         NvmeRequest *req)
{
    ...
    // maxlat = zns_advance_status(n, ns, cmd, req);
    uint64_t curlat = 0, maxlat = 0;
    uint32_t lpn = 0, slpn = 0, elpn = 0;
    ...
    for (lpn = slpn; lpn <= elpn; lpn++) {
        ...
        curlat = zns_advance_status(n->zns, &ppa, &swr);
        maxlat = (curlat > maxlat) ? curlat : maxlat;
    }
                          ...
                          NvmeRequest *req)
{
    ...
    slpn = slba / zns->sp.secs_per_pg;
    elpn = (slba + nlb - 1) / zns->sp.secs_per_pg;
    for (lpn = slpn; lpn <= elpn; lpn++) {
        struct ppa ppa = lpn_to_ppa(ns, lpn);
        struct nand_cmd swr;
        swr.type = USER_IO;
...
static void zns_init(FemuCtrl *n, Error **errp)
{
    ...
    // qemu_thread_create(&n->zns->ftl_thread, "FEMU-ZNS-FTL-Thread", zns_ftl_thread, n,
...
static void *zns_ftl_thread(void *arg)
{
    ...
    while (1) {
        for (i = 1; i <= n->nr_pollers; i++) {
            ...
            ftl_assert(req);
            cmd =  (NvmeRwCmd*)&req->cmd;

            switch (req->cmd.opcode) {
            case NVME_CMD_READ:
                zns_read(n, ns, cmd, req);
                break;
            case NVME_CMD_WRITE:
            ...
            case NVME_CMD_DSM:
                ...
                break;
            case NVME_CMD_ZONE_APPEND:
                // fprintf(stdout, "Rzone zns append\n");
                break;
            default:
