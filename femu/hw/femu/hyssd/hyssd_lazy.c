#include "hyssd_lazy.h"

#define MIN_DISCARD_GRANULARITY     (4 * KiB)
#define NVME_DEFAULT_ZONE_CAP_SIZE  (256 * MiB)
#define NVME_DEFAULT_ZONE_SIZE      (256 * MiB)
...
static void hy_ssd_init_params(struct hyparams *spp, FemuCtrl *n)
{
    ...
    // random zone pages
    spp->tt_rpgs = spp->pgs_per_blk * spp->num_rzones * spp->pls_per_lun 
                    * spp->luns_per_ch * spp->nchs;
    
    spp->tt_rpgs = spp->tt_pgs;
...
static int hy_ssd_init_zone_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    // single plane
    fprintf(stdout, "%d %d %d %d %d\n", spp->nchs, spp->luns_per_ch, spp->pgs_per_blk, spp->secs_per_pg, spp->secsz);
...
static void hy_ssd_init_zoned_state(NvmeNamespace *ns)
{
    ...
    for (i = 0; i < n->num_zones; i++, zone++) {
        ...
        zone->line_idx = -1; // unmapped (lazy allocation)
        zone->dirty = 0;
...
static void hy_zns_aio_zone_reset_cb(NvmeRequest *req, NvmeZone *zone)
{
    ...
    // femu_log("hy_zns_aio_zone_reset_cb, state=0x%x\n", hy_zns_get_zone_state(zone));
    switch (hy_zns_get_zone_state(zone)) {
    case NVME_ZONE_STATE_EXPLICITLY_OPEN:
        /* fall through */
    case NVME_ZONE_STATE_IMPLICITLY_OPEN:
    ...
    case NVME_ZONE_STATE_FULL:
        ...
        // assert(zone->line_idx != -1);
        if (zone->line_idx != -1) {
            struct hyssd *hyssd = ns->ctrl->hyssd;
            struct line_mgmt *lm = &hyssd->lm;
            struct line *line = &lm->lines[zone->line_idx];

            fprintf(stdout, "Reset : zone #%d - line #%d\n", line->zone_idx, zone->line_idx);
            ...
            fprintf(stdout, "[Error] Reset: zone->line_idx == -1\n");
        }
...
static uint16_t hy_zns_reset_zone(NvmeNamespace *ns, NvmeZone *zone,
                                  NvmeZoneState state, NvmeRequest *req)
{
    // femu_log("hy_zns_reset_zone, state=0x%x\n", state);
    switch (state) {
    case NVME_ZONE_STATE_EMPTY:
        return NVME_SUCCESS;
    case NVME_ZONE_STATE_EXPLICITLY_OPEN:
...
static uint16_t hy_zns_zone_mgmt_send(FemuCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_ZONE_ACTION_RESET:
        ...
        *resets = 1;
        // uint64_t maxlat = hy_ssd_advance_status(n, ns, cmd, req);
...
static int hy_ssd_avail_free_lines(struct hyssd *hyssd)
{
    return hyssd->lm.free_line_cnt > 0;
}
                            ...
                            NvmeRequest *req)
{
    ...
    for (lpn = slpn; lpn <= elpn; lpn++) {
        struct ppa ppa = lpn_to_ppa(ns, lpn);
        struct nand_cmd swr;
        swr.type = USER_IO;
        swr.cmd = NAND_READ;
        swr.stime = req->stime;
...
RETRY:
    if (zone->line_idx == -1) {
        struct line *line = hy_get_next_free_line(n->hyssd);
        if (line == NULL) {
            hy_do_gc(n->hyssd, true);
            goto RETRY;
        }
        line->is_rnd = 0;
        line->zone_idx = hy_zns_zone_idx(ns, slba);
        zone->line_idx = line->id;
        fprintf(stdout, "Zone #%d -> line #%d\n", line->zone_idx, zone->line_idx);
    }
                           ...
                           NvmeRequest *req)
{
    ...
    uint16_t res = NVME_INVALID_OPCODE | NVME_DNR;
...
{
    fprintf(stdout, "hyssd-lazy init\n");
    NvmeNamespace *ns = &n->namespaces[0];
