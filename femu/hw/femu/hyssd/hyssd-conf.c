#include "hyssd-conf.h"

#define MIN_DISCARD_GRANULARITY     (4 * KiB)
#define NVME_DEFAULT_ZONE_CAP_SIZE  (1024 * MiB)
#define NVME_DEFAULT_ZONE_SIZE      (1024 * MiB)
#define NVME_DEFAULT_MAX_AZ_SIZE    (128 * KiB)
#define NVME_ZNS_MAX_ACTIVE_ZONES   14
#define NVME_ZNS_MAX_OPEN_ZONES     14
...
/* === Line-group helper functions === */

static inline int get_parallel_groups(struct hyparams *spp)
{
    return spp->parallel_groups;
}

static inline int get_line_set(int line_id, struct hyparams *spp)
{
    int parallel_groups = get_parallel_groups(spp);
    return line_id / parallel_groups;
}

static inline int get_line_blk_start(int line_id, struct hyparams *spp)
{
    int line_set = get_line_set(line_id, spp);
    return line_set * spp->blks_per_position;
}

static inline int get_line_blk_end(int line_id, struct hyparams *spp)
{
    return get_line_blk_start(line_id, spp) + spp->blks_per_position;
}

static inline int get_group_id_from_position(int ch, int lun, struct hyparams *spp)
{
    int ch_group = ch / spp->chs_per_line;
    int lun_group = lun / spp->luns_per_line;
    int lun_groups = spp->luns_per_ch / spp->luns_per_line;
    return ch_group * lun_groups + lun_group;
}

static inline int get_line_id_from_ppa(struct ppa *ppa, struct hyparams *spp)
{
    int parallel_groups = get_parallel_groups(spp);
    int line_set = ppa->g.blk / spp->blks_per_position;
    int group_id = get_group_id_from_position(ppa->g.ch, ppa->g.lun, spp);
    return line_set * parallel_groups + group_id;
}

/* === End line-group helpers === */
...
static uint64_t ppa2pgidx(struct hyssd *ssd, struct ppa *ppa)
{
    ...
    if (line_id < 0 || line_id >= lm->tt_lines) {
        ftl_err("Invalid PPA (blk=%d, ch=%d, lun=%d) -> line_id=%d, tt_lines=%d\n",
                ppa->g.blk, ppa->g.ch, ppa->g.lun, line_id, lm->tt_lines);
        return UINT64_MAX;
    }
    ...
    /* Calculate relative ch/lun within this line's range */
    int rel_ch = ppa->g.ch - line->ch_start;
    int rel_lun = ppa->g.lun - line->lun_start;
    ftl_assert(rel_ch >= 0 && rel_ch < line->ch_cnt);
    ftl_assert(rel_lun >= 0 && rel_lun < line->lun_cnt);

    int blk_start = get_line_blk_start(line_id, spp);
    int blk_offset = ppa->g.blk - blk_start;
    ftl_assert(blk_offset >= 0 && blk_offset < spp->blks_per_position);

    int pos_in_line = blk_offset * (line->ch_cnt * line->lun_cnt) +
                      rel_ch * line->lun_cnt + rel_lun;
    uint64_t pgidx = (uint64_t)line->id * spp->pgs_per_line +
                     pos_in_line * spp->pgs_per_blk + ppa->g.pg;

    ftl_assert(pgidx < spp->max_rpgs);
    return pgidx;
}
...
// line == sub-zone unit (zone is composed of lines_per_zone lines)
static void hy_ssd_init_lines(FemuCtrl *n, struct hyssd *ssd)
{
    struct hyparams *spp = &ssd->sp;
    struct line_mgmt *lm = &ssd->lm;
    struct line *line;
    int rzone_lines = spp->num_rzones * spp->lines_per_zone;
    int parallel_groups = spp->parallel_groups;
    int ch_groups = spp->nchs / spp->chs_per_line;
    ...
    for (int i = 0; i < lm->tt_lines; i++) {
        ...
        /* Assign ch/lun range based on group_id.
         * Must match get_group_id_from_position():
         *   group_id = ch_group * lun_groups + lun_group */
        int group_id = i % parallel_groups;
        int lun_groups = spp->luns_per_ch / spp->luns_per_line;
        int ch_group_idx = group_id / lun_groups;
        int lun_group_idx = group_id % lun_groups;

        line->ch_start = ch_group_idx * spp->chs_per_line;
        line->ch_cnt = spp->chs_per_line;
        line->lun_start = lun_group_idx * spp->luns_per_line;
        line->lun_cnt = spp->luns_per_line;
        ...
        if (i < rzone_lines + spp->num_op_line) {
            line->is_rnd = 1;
    ...
    ftl_assert(lm->free_line_cnt == rzone_lines + spp->num_op_line);
    lm->victim_line_cnt = 0;
    lm->full_line_cnt = 0;

    fprintf(stdout, "\n=== [hy_ssd_init_lines] Line Allocation ===\n");
    fprintf(stdout, "  tt_lines=%d, parallel_groups=%d, lines_per_zone=%d\n",
            lm->tt_lines, parallel_groups, spp->lines_per_zone);
    fprintf(stdout, "  rzone_lines=%d, free=%d, victim=%d, full=%d\n",
            rzone_lines, lm->free_line_cnt, lm->victim_line_cnt, lm->full_line_cnt);

    /* Print first few line-group mappings for verification */
    int show = (lm->tt_lines < 16) ? lm->tt_lines : 16;
    fprintf(stdout, "  --- First %d lines (group_id verification) ---\n", show);
    for (int i = 0; i < show; i++) {
        line = &lm->lines[i];
        int gid = i % parallel_groups;
        int blk_s = get_line_blk_start(i, spp);
        /* Verify: pick first (ch,lun) of this line and check get_group_id */
        int verify_gid = get_group_id_from_position(line->ch_start, line->lun_start, spp);
        fprintf(stdout, "    line %4d: group=%d ch=[%2d,%2d) lun=[%2d,%2d) blk=%3d is_rnd=%d verify=%s\n",
                i, gid, line->ch_start, line->ch_start + line->ch_cnt,
                line->lun_start, line->lun_start + line->lun_cnt,
                blk_s, line->is_rnd,
                (verify_gid == gid) ? "OK" : "MISMATCH!");
    }
    fprintf(stdout, "=== End Line Allocation ===\n\n");
}
...
static void hy_ssd_init_write_pointer(struct hyssd *ssd)
{
    ...
    wpp->curline = curline;
    wpp->ch = curline->ch_start;
    wpp->lun = curline->lun_start;
    wpp->pl = 0;
    wpp->blk = get_line_blk_start(curline->id, spp);
    wpp->pg = 0;
}
...
static void hy_ssd_advance_write_pointer(struct hyssd *ssd)
{
    ...
    int ch_end = curline->ch_start + curline->ch_cnt;
    int lun_end = curline->lun_start + curline->lun_cnt;
    ...
    if (wpp->ch == ch_end) {
        ...
        if (wpp->lun == lun_end) {
            ...
            if (wpp->pg == spp->pgs_per_blk) {
                ...
                if (wpp->blk >= blk_end) {
                    /* Line is full, move to victim or full list */
...
static void hy_ssd_init_params(struct hyparams *spp, FemuCtrl *n)
{
    ...
    // random zone pages (in zone units)
    spp->pgs_per_zone = (NVME_DEFAULT_ZONE_SIZE / spp->secsz) / spp->secs_per_pg;
    spp->tt_rpgs = spp->num_rzones * spp->pgs_per_zone;

    /* max_rzones determines maptbl/rmap pre-allocation size.
     * 0 (default) means use num_zones (all zones could become R-zones). */
    int max_rzones = n->sp.max_rzones;
    if (max_rzones <= 0) {
        /* Will be resolved after num_zones is calculated in zone_geometry.
         * For now, use a safe upper bound from total pages. */
        max_rzones = spp->tt_pgs / spp->pgs_per_zone;
    }
    if (max_rzones < spp->num_rzones) {
        max_rzones = spp->num_rzones;
    }
    spp->max_rpgs = max_rzones * spp->pgs_per_zone;
    ...
    /* Configurable parallelism per line */
    spp->chs_per_line = n->bb_params.chs_per_line;
    spp->luns_per_line = n->bb_params.luns_per_line;
    /* If not set or 0, default to full parallelism */
    if (spp->chs_per_line <= 0) spp->chs_per_line = spp->nchs;
    if (spp->luns_per_line <= 0) spp->luns_per_line = spp->luns_per_ch;
    ftl_assert(spp->nchs % spp->chs_per_line == 0);
    ftl_assert(spp->luns_per_ch % spp->luns_per_line == 0);

    int ch_groups = spp->nchs / spp->chs_per_line;
    int lun_groups = spp->luns_per_ch / spp->luns_per_line;
    spp->parallel_groups = ch_groups * lun_groups;
    spp->lines_per_zone = spp->parallel_groups;
    spp->blks_per_position = 1;

    spp->blks_per_line = spp->chs_per_line * spp->luns_per_line * spp->blks_per_position;
    spp->pgs_per_line = spp->blks_per_line * spp->pgs_per_blk;
    spp->secs_per_line = spp->pgs_per_line * spp->secs_per_pg;
    spp->tt_lines = spp->blks_per_lun * spp->parallel_groups;
    spp->tt_lines += spp->num_op_line;

    /* GC thresholds based on rzone line count */
    int rzone_lines = spp->num_rzones * spp->lines_per_zone;
    spp->gc_thres_pcent = 0.75;
    spp->gc_thres_lines = (int)((1 - spp->gc_thres_pcent) * rzone_lines);
    ...
    spp->gc_thres_lines_high = (int)((1 - spp->gc_thres_pcent_high) * rzone_lines);
    ...
    fprintf(stdout, "\n=== [hy_ssd_init_params] HYSSD Configuration ===\n");
    fprintf(stdout, "  Device: %dch x %dway, secsz=%d, secs_per_pg=%d, pgs_per_blk=%d\n",
            spp->nchs, spp->luns_per_ch, spp->secsz, spp->secs_per_pg, spp->pgs_per_blk);
    fprintf(stdout, "  blks_per_pl=%d, pls_per_lun=%d\n", spp->blks_per_pl, spp->pls_per_lun);
    fprintf(stdout, "  --- Line Geometry ---\n");
    fprintf(stdout, "  chs_per_line=%d, luns_per_line=%d\n", spp->chs_per_line, spp->luns_per_line);
    fprintf(stdout, "  parallel_groups=%d, lines_per_zone=%d, blks_per_position=%d\n",
            spp->parallel_groups, spp->lines_per_zone, spp->blks_per_position);
    fprintf(stdout, "  blks_per_line=%d, pgs_per_line=%d, secs_per_line=%d\n",
            spp->blks_per_line, spp->pgs_per_line, spp->secs_per_line);
    int line_size_mb = (int)((uint64_t)spp->secs_per_line * spp->secsz / (1024 * 1024));
    fprintf(stdout, "  line_size=%d MiB\n", line_size_mb);
    fprintf(stdout, "  tt_lines=%d (blks_per_lun=%d x parallel_groups=%d)\n",
            spp->tt_lines, spp->blks_per_lun, spp->parallel_groups);
    fprintf(stdout, "  --- Zone Geometry ---\n");
    fprintf(stdout, "  zone_size=1024 MiB (NVME_DEFAULT_ZONE_SIZE)\n");
    fprintf(stdout, "  pgs_per_zone=%d, num_rzones=%d\n", spp->pgs_per_zone, spp->num_rzones);
    fprintf(stdout, "  --- Rzone ---\n");
    fprintf(stdout, "  rzone_lines=%d (num_rzones=%d x lines_per_zone=%d)\n",
            rzone_lines, spp->num_rzones, spp->lines_per_zone);
    int rzone_mb = rzone_lines * line_size_mb;
    fprintf(stdout, "  rzone_capacity=%d MiB (%d GiB)\n", rzone_mb, rzone_mb / 1024);
    fprintf(stdout, "  tt_rpgs=%d (maptbl/rmap size)\n", spp->tt_rpgs);
    fprintf(stdout, "  physical_rpgs=%d (rzone_lines x pgs_per_line)\n",
            rzone_lines * spp->pgs_per_line);
    fprintf(stdout, "  --- GC ---\n");
    fprintf(stdout, "  gc_thres_lines=%d (trigger), gc_thres_lines_high=%d (aggressive)\n",
            spp->gc_thres_lines, spp->gc_thres_lines_high);
    fprintf(stdout, "=== End Configuration ===\n\n");
}
...
static void hy_ssd_init_maptbl(struct hyssd *ssd)
{
    ...
    /* Allocate for max_rpgs so modify-zone can grow tt_rpgs without realloc */
    ssd->maptbl = g_malloc0(sizeof(struct ppa) * spp->max_rpgs);
    for (int i = 0; i < spp->max_rpgs; i++) {
...
static void hy_ssd_init_rmap(struct hyssd *ssd)
{
    ...
    ssd->rmap = g_malloc0(sizeof(uint64_t) * spp->max_rpgs);
    for (int i = 0; i < spp->max_rpgs; i++) {
...
{
    int line_id = get_line_id_from_ppa(ppa, &ssd->sp);
    return &(ssd->lm.lines[line_id]);
}
...
static uint64_t hy_ssd_advance_status(struct hyssd *ssd, struct ppa *ppa, struct nand_cmd *ncmd)
{
    ...
    uint64_t nand_stime;
    struct hyparams *spp = &ssd->sp;
    struct hyssd_ch *ch = NULL;
    struct hyssd_lun *lun = NULL;
    uint64_t lat = 0;
...
static int hy_do_gc(struct hyssd *ssd, bool force)
{
    ...
    int ch_start = victim_line->ch_start;
    int ch_end = ch_start + victim_line->ch_cnt;
    int lun_start = victim_line->lun_start;
    int lun_end = lun_start + victim_line->lun_cnt;
    int blk_start = get_line_blk_start(victim_line->id, spp);
    int blk_end = get_line_blk_end(victim_line->id, spp);

    ftl_log("GC-ing line:%d,vpc=%d,ipc=%d,ch=[%d-%d),lun=[%d-%d),blk=[%d-%d),"
            "victim=%d,full=%d,free=%d\n",
            victim_line->id, victim_line->vpc, victim_line->ipc,
            ch_start, ch_end, lun_start, lun_end, blk_start, blk_end,
    ...
    ftl_log("Rzone GC finished cnt : %lu\n", ssd->sp.gc_count);
    return 0;
}
...
static int hy_ssd_init_zone_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    fprintf(stdout, "\n=== [hy_ssd_init_zone_geometry] Zone Geometry ===\n");
    fprintf(stdout, "  zone_size=%lu LBAs (%lu MiB), zone_capacity=%lu LBAs\n",
            (unsigned long)n->zone_size, (unsigned long)(zone_size / (1024*1024)),
            (unsigned long)n->zone_capacity);
    fprintf(stdout, "  num_zones=%u, num_rzones=%u, num_szones=%u\n",
            n->num_zones, n->num_rzone, n->num_zones - n->num_rzone);
    fprintf(stdout, "  pgs_per_zone=%d, lines_per_zone=%d\n",
            n->hyssd->sp.pgs_per_zone, n->hyssd->sp.lines_per_zone);
    fprintf(stdout, "  ns_size=%lu MiB\n", (unsigned long)(ns->size / (1024*1024)));
    fprintf(stdout, "=== End Zone Geometry ===\n");
...
static void hy_ssd_init_zoned_state(NvmeNamespace *ns)
{
    ...
    for (i = 0; i < n->num_zones; i++, zone++) {
        if (i < num_rzone)
            ...
            zone->rnd = 0;
            zone->line_idx = i * n->hyssd->sp.lines_per_zone;
        }
...
static struct ppa lpn_to_ppa(NvmeNamespace *ns, uint64_t lpn)
{
    FemuCtrl *n = ns->ctrl;
    struct hyssd *hyssd = n->hyssd;
    uint64_t nchs = hyssd->sp.nchs;
    uint64_t nluns = hyssd->sp.luns_per_ch;
...
static int handle_zone_decrease(FemuCtrl *n, uint32_t new_rzone, uint32_t old_rzone)
{
    ...
    for (victim_zone = new_rzone; victim_zone < old_rzone; victim_zone++) {
        ...
        for (uint32_t lpn = victim_start_lpn; lpn < victim_end_lpn; lpn++) {
            ...
            if (mapped_ppa(&ppa)) { // JM: verify it should be UNMAPPED here (interacts with DSM).
                fprintf(stdout, "Error: Zone %u still has valid pages, cannot convert to sequential (lpn:%u)\n", victim_zone, lpn);
                hy_mark_page_invalid(n->hyssd, &ppa);
                set_rmap_ent(n->hyssd, INVALID_LPN, &ppa);
                return NVME_ZONE_INVAL_TRANSITION;
            }
        }
...
/* Line management - moves lines_per_zone lines per zone conversion */
static uint16_t handle_line_management(FemuCtrl *n, uint32_t old_rzone, uint32_t new_rzone, bool is_increase)
{
    ...
    NvmeZone *zones = n->zone_array;
    uint32_t victim_zone;
    int lpz = spp->lines_per_zone;

    if (is_increase) {
        /* szone -> rzone: free all lines_per_zone lines per zone */
        for (victim_zone = old_rzone; victim_zone < new_rzone; victim_zone++) {
            int first_line = zones[victim_zone].line_idx;
            for (int j = 0; j < lpz; j++) {
                line = &lm->lines[first_line + j];
                line->is_rnd = 1;
                line->pos = 0;
                line->ipc = 0;
                line->vpc = 0;
        ...
        /* rzone -> szone: allocate lines_per_zone contiguous lines per zone.
         * The increase path accesses lines as line_idx + j, so we must
         * guarantee that all lpz lines are contiguous and aligned by id.
         *
         * Strategy: scan all aligned line groups, pick the one with lowest
         * total vpc (cheapest to GC), then force-GC any non-free lines
         * in that group before taking ownership. */
        for (victim_zone = new_rzone; victim_zone < old_rzone; victim_zone++) {
            int best_base = -1;
            int best_cost = INT_MAX; /* total vpc across the group */

            /* 1. Score each aligned group of lpz lines */
            int rzone_lines = spp->num_rzones * lpz + spp->num_op_line;
            for (int base = 0; base + lpz <= rzone_lines; base += lpz) {
                int cost = 0;
                bool usable = true;
                for (int j = 0; j < lpz; j++) {
                    line = &lm->lines[base + j];
                    if (!line->is_rnd) {
                        /* Already assigned to another S-zone */
                        usable = false;
                        break;
                    }
                    cost += line->vpc;
                }
                if (usable && cost < best_cost) {
                    best_cost = cost;
                    best_base = base;
                }
            }
            if (best_base == -1) {
                femu_err("No usable line group for S-zone (need %d contiguous)\n", lpz);
                return NVME_ZONE_INVAL_TRANSITION;
            }

            fprintf(stdout, "modify_rzone: best group base=%d, cost(vpc)=%d\n",
                    best_base, best_cost);

            /* 2. Force-GC non-free lines in the chosen group */
            for (int j = 0; j < lpz; j++) {
                line = &lm->lines[best_base + j];

                if (line->vpc == 0 && line->ipc == 0) {
                    /* Already free — just remove from free list */
                    continue;
                }

                /* Line has data: GC it (copy valid pages out, erase, mark free).
                 * First remove from whichever list/pq it's in. */
                if (line->vpc == spp->pgs_per_line) {
                    /* In full_line_list */
                    QTAILQ_REMOVE(&lm->full_line_list, line, entry);
                    lm->full_line_cnt--;
                ...
                /* else: curline (write pointer) — should not normally happen
                 * since we pick the cheapest group, but handle gracefully */

                struct hyssd *ssd = n->hyssd;
                int ch_start = line->ch_start;
                int ch_end = ch_start + line->ch_cnt;
                int lun_start = line->lun_start;
                int lun_end = lun_start + line->lun_cnt;
                int blk_start = get_line_blk_start(line->id, spp);
                int blk_end = get_line_blk_end(line->id, spp);
                struct ppa ppa;

                ssd->sp.gc_count++;
                for (int blk = blk_start; blk < blk_end; blk++) {
                    for (int ch = ch_start; ch < ch_end; ch++) {
                        for (int ln = lun_start; ln < lun_end; ln++) {
                            ppa.ppa = 0;
                            ppa.g.ch = ch;
                            ppa.g.lun = ln;
                            ppa.g.blk = blk;
                            ppa.g.pl = 0;
                            /* Force-GC: line may be partially written
                             * (e.g. curline), so skip free pages unlike
                             * hy_clean_one_block which asserts none free. */
                            for (int pg = 0; pg < spp->pgs_per_blk; pg++) {
                                ppa.g.pg = pg;
                                struct hyssd_pg *pgp = get_pg(ssd, &ppa);
                                if (pgp->status == PG_FREE)
                                    continue;
                                if (pgp->status == PG_VALID) {
                                    hy_gc_read_page(ssd, &ppa);
                                    hy_gc_write_page(ssd, &ppa);
                                    ssd->sp.gc_pgs++;
                                }
            ...
            /* 3. Take all lpz lines from free list for this S-zone */
            for (int j = 0; j < lpz; j++) {
                line = &lm->lines[best_base + j];
            ...
            zones[victim_zone].line_idx = best_base;
        }
    }
...
static int verify_mapping_tables(FemuCtrl *n)
{
    struct hyssd *ssd = n->hyssd;
    struct hyparams *spp = &ssd->sp;
    uint32_t max_lpn = spp->tt_rpgs;
    uint64_t pgidx;
    int errors = 0;
    
    fprintf(stdout, "Verifying mapping tables for %u pages (tt_rpgs)\n", max_lpn);
                              ...
                              struct nvme_victim_idx **vidx) 
{
    ...
    /* Update tt_rpgs (maptbl/rmap are pre-allocated for max_rpgs at init) */
    {
        struct hyparams *spp = &n->hyssd->sp;
        uint32_t new_rpgs = new_rzone * ppz;

        if (new_rpgs > spp->max_rpgs) {
            femu_err("new_rpgs %u exceeds max_rpgs %d (increase max_rzones)\n",
                     new_rpgs, spp->max_rpgs);
            return NVME_CAP_EXCEEDED;
        }
        spp->tt_rpgs = new_rpgs;
    }
    ...
    /* Update num_rzones and recalculate GC thresholds for new rzone count.
     * gc_thres_lines is compared against free_line_cnt (line count),
     * so threshold must be based on rzone_lines, not rzone count. */
    {
        struct hyparams *spp = &n->hyssd->sp;
        int new_rzone_lines = new_rzone * spp->lines_per_zone;
        spp->num_rzones = new_rzone;
        spp->gc_thres_lines = (int)((1.0 - spp->gc_thres_pcent) * new_rzone_lines);
        if (spp->gc_thres_lines == 0)
            spp->gc_thres_lines = 1;
        spp->gc_thres_lines_high = (int)((1.0 - spp->gc_thres_pcent_high) * new_rzone_lines);
        if (spp->gc_thres_lines_high == 0)
            spp->gc_thres_lines_high = 1;

        fprintf(stdout, "modify_rzone: updated num_rzones=%u, rzone_lines=%d, "
                "gc_thres=%d, gc_thres_high=%d, free_lines=%d\n",
                spp->num_rzones, new_rzone_lines, spp->gc_thres_lines,
                spp->gc_thres_lines_high, lm->free_line_cnt);
    }
    return NVME_SUCCESS;
}
...
static uint16_t hy_zns_zone_mgmt_send(FemuCtrl *n, NvmeRequest *req)
{
    ...
    struct hyssd *hyssd = n->hyssd;
    struct ppa ppa;
    // uint64_t rzone_limit = n->hyssd->sp.num_rzones * n->zone_size;
    ...
    case NVME_ZONE_ACTION_FINISH:
        ...
        fprintf(stdout, "Szone FINISH SUCCESS [slba : %ld, zidx : %u]\n", slba, hy_zns_zone_idx(ns, slba));
        break;
    case NVME_ZONE_ACTION_RESET:
        ...
        *resets = 1;
        for (int i = 0; i < hyssd->sp.nchs; i++) {
            for (int j = 0; j < hyssd->sp.luns_per_ch; j++) {
                ppa.ppa = 0;
                ppa.g.ch = i;
                ppa.g.lun = j;
        ...
        (*resets)--;
        fprintf(stdout, "Szone RESET SUCCESS [slba : %ld, zidx : %u]\n", slba, hy_zns_zone_idx(ns, slba));
        break;
...
static uint16_t hyssd_id_dev(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    dev_data.hyssd.num_zones = n->num_zones;
...
static uint16_t hy_zns_mon_hyssd(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    *zr_count_ptr = hyssd->sp.zr_count;
    ...
    for (int i = 0; i < lm->tt_lines; i++) {
        ...
        line_infos[i].ridx = 0;
        line_infos[i].ch_start = line->ch_start;
        line_infos[i].ch_cnt = line->ch_cnt;
        line_infos[i].lun_start = line->lun_start;
        line_infos[i].lun_cnt = line->lun_cnt;
    }
    ptr += line_info_size;
...
static uint16_t hy_ssd_reset(FemuCtrl *n)
{
    struct hyssd *hyssd = n->hyssd;
    struct hyparams *spp = &hyssd->sp;
    struct line_mgmt *lm = &hyssd->lm;
    uint32_t orig_num_rzones = n->sp.num_rzones;
    int rzone_lines = orig_num_rzones * spp->lines_per_zone;
    int i, j, k, m;

    fprintf(stdout, "HYSSD: Device reset started...\n");

    /* 1. Restore original num_rzones (may have been changed by modify-zone) */
    spp->num_rzones = orig_num_rzones;
    spp->tt_rpgs = orig_num_rzones * spp->pgs_per_zone;
    n->num_rzone = orig_num_rzones;

    /* Recalculate GC thresholds for original rzone count */
    spp->gc_thres_lines = (int)((1.0 - spp->gc_thres_pcent) * rzone_lines);
    if (spp->gc_thres_lines == 0)
        spp->gc_thres_lines = 1;
    spp->gc_thres_lines_high = (int)((1.0 - spp->gc_thres_pcent_high) * rzone_lines);
    if (spp->gc_thres_lines_high == 0)
        spp->gc_thres_lines_high = 1;

    /* 2. Reset NAND state: channels -> LUNs -> blocks -> pages */
    for (i = 0; i < spp->nchs; i++) {
        struct hyssd_ch *ch = &hyssd->ch[i];
        ch->next_ch_avail_time = 0;
        ch->busy = 0;
        for (j = 0; j < spp->luns_per_ch; j++) {
            struct hyssd_lun *lun = &ch->lun[j];
            lun->next_lun_avail_time = 0;
            lun->busy = false;
            lun->gc_endtime = 0;
            for (k = 0; k < spp->pls_per_lun; k++) {
                struct hyssd_pl *pl = &lun->pl[k];
                for (m = 0; m < spp->blks_per_pl; m++) {
                    struct hyssd_blk *blk = &pl->blk[m];
                    blk->ipc = 0;
                    blk->vpc = 0;
                    blk->erase_cnt = 0;
                    blk->wp = 0;
                    for (int p = 0; p < spp->pgs_per_blk; p++) {
                        struct hyssd_pg *pg = &blk->pg[p];
                        pg->status = PG_FREE;
                        for (int s = 0; s < spp->secs_per_pg; s++) {
                            pg->sec[s] = SEC_FREE;
                        }
                    }
                }
    ...
    /* 3. Reset mapping tables (clear full max_rpgs range) */
    for (i = 0; i < spp->max_rpgs; i++) {
        hyssd->maptbl[i].ppa = UNMAPPED_PPA;
        hyssd->rmap[i] = INVALID_LPN;
    }

    /* 4. Reset line management */
    ...
    /* 5. Reset write pointer */
    hy_ssd_init_write_pointer(hyssd);

    /* 6. Reset zone state (restore original R/S zone assignment and line_idx) */
    QTAILQ_INIT(&n->exp_open_zones);
    QTAILQ_INIT(&n->imp_open_zones);
    QTAILQ_INIT(&n->closed_zones);
    QTAILQ_INIT(&n->full_zones);
    ...
    for (i = 0; i < n->num_zones; i++) {
        ...
        if (i < orig_num_rzones) {
            zone->rnd = 1;
        } else {
            zone->rnd = 0;
            zone->line_idx = i * spp->lines_per_zone;
        }
        zone->dirty = 0;
    ...
    fprintf(stdout, "HYSSD: Device reset completed (factory state)\n");
    fprintf(stdout, "  num_rzones=%u, num_szones=%u, rzone_lines=%d, free_lines=%d\n",
            orig_num_rzones, n->num_zones - orig_num_rzones,
            rzone_lines, lm->free_line_cnt);
    return NVME_SUCCESS;
}
...
static uint16_t hy_ssd_write(struct hyssd *ssd, NvmeNamespace *ns, NvmeRequest *req)
{
    ...
    if (end_lpn >= spp->tt_rpgs) {
        ftl_err("Rzone WRITE OOB: start_lpn=%"PRIu64", end_lpn=%"PRIu64", tt_rpgs=%d\n",
                start_lpn, end_lpn, spp->tt_rpgs);
        return NVME_SUCCESS;
    }
...
err:
    femu_err("*********ZONE WRITE FAILED [ZIDX : %d SLBA : %lu NLB : %u WP : %lu]*********\n", hy_zns_zone_idx(ns, slba), slba, nlb, zone->w_ptr);
    return status | NVME_DNR;
}
                           ...
                           NvmeRequest *req)
{
    ...
    case NVME_CMD_READ:
        if(slba + nlb <= rzone_limit)
            ...
            res = hy_zns_read(n, ns, rw, req);
        break;
    case NVME_CMD_WRITE:
        if(slba + nlb <= rzone_limit)
            res = hy_ssd_write(n->hyssd, ns, req);
        else
            res = hy_zns_write(n, ns, rw, req);
        break;
