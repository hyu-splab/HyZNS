...
static uint64_t ppa2pgidx(struct hyssd *ssd, struct ppa *ppa)
{
    ...
    struct line *line = &lm->lines[ppa->g.blk];
...
static void hy_ssd_init_lines(FemuCtrl *n, struct hyssd *ssd)
{
    ...
    // blks_per_pl : whole device
    // num_rzones : rzone region
    // tt_lines covers all (R + S) zones, not just rzones
    lm->tt_lines = spp->blks_per_pl + spp->num_op_line;
    ...
    for (int i = 0; i < lm->tt_lines; i++) {
        line = &lm->lines[i];
        line->id = i; // line id == block number
        line->ipc = 0;
        line->vpc = 0;
        line->pos = 0;
        if (i < spp->rnd_lines) {
            ...
            /* initialize all rzone lines as free lines */
    ...
    ftl_assert(lm->free_line_cnt == num_rzone + spp->num_op_line);
    lm->victim_line_cnt = 0;
    lm->full_line_cnt = 0;
    
    fprintf(stdout, "[%s] free : %d victim : %d full : %d\n", __func__, 
...
static void hy_ssd_init_params(struct hyparams *spp, FemuCtrl *n)
{
    ...
    spp->tt_rpgs = (spp->num_rzones + spp->num_op_line) * spp->pgs_per_zone;
    ...
    spp->rnd_lines = spp->num_rzones + spp->num_op_line;
...
static int hy_do_gc(struct hyssd *ssd, bool force)
{
    ...
    ftl_log("Rzone GC finished cnt : %d\n", ssd->sp.gc_count);
    return 0;
}
                              ...
                              struct nvme_victim_idx **vidx) 
{
    ...
    /* Update num_rzones and recalculate GC thresholds for new rzone count */
    {
        ...
        spp->gc_thres_lines = (int)((1.0 - spp->gc_thres_pcent) * new_rzone);
        if (spp->gc_thres_lines == 0)
            spp->gc_thres_lines = 1;
        spp->gc_thres_lines_high = (int)((1.0 - spp->gc_thres_pcent_high) * new_rzone);
        if (spp->gc_thres_lines_high == 0)
            spp->gc_thres_lines_high = 1;

        fprintf(stdout, "modify_rzone: updated num_rzones=%u, gc_thres=%d, "
                "gc_thres_high=%d, free_lines=%d\n",
                spp->num_rzones, spp->gc_thres_lines,
...
static uint16_t hy_zns_zone_mgmt_recv(FemuCtrl *n, NvmeRequest *req)
{
    ...
    if (zra == NVME_ZONE_REPORT_RZONE) {
        for (int i = 0; i < n->num_zones/64; ++i) {
            zone = &n->zone_array[i];
        ...
        for (int i = 0; i < n->hyssd->lm.tt_lines/64; ++i) {
        ...
        for (int lpn = 0; lpn < n->hyssd->sp.tt_rpgs/64; lpn += 32768) {
...
static uint16_t hy_ssd_reset(FemuCtrl *n)
{
    ...
    spp->tt_rpgs = (orig_num_rzones + spp->num_op_line) * spp->pgs_per_zone;
    spp->rnd_lines = orig_num_rzones + spp->num_op_line;
    n->num_rzone = orig_num_rzones;

    /* Recalculate GC thresholds for original rzone count */
    spp->gc_thres_lines = (int)((1.0 - spp->gc_thres_pcent) * orig_num_rzones);
    if (spp->gc_thres_lines == 0)
        spp->gc_thres_lines = 1;
    spp->gc_thres_lines_high = (int)((1.0 - spp->gc_thres_pcent_high) * orig_num_rzones);
    if (spp->gc_thres_lines_high == 0)
        spp->gc_thres_lines_high = 1;
    ...
    /* 3. Reset mapping tables */
    for (i = 0; i < spp->tt_rpgs; i++) {
    ...
    fprintf(stdout, "  num_rzones=%u, num_szones=%u, free_lines=%d\n",
            orig_num_rzones, n->num_zones - orig_num_rzones, lm->free_line_cnt);
    return NVME_SUCCESS;
}
