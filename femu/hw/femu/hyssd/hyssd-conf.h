...
typedef struct line {
    int id;  /* line id (unique across all lines) */
    ...
    int zone_idx;
    int ch_start;   /* starting channel for this line's group */
    int ch_cnt;     /* number of channels in this line */
    int lun_start;  /* starting LUN for this line's group */
    int lun_cnt;    /* number of LUNs in this line */
} line;
...
typedef struct {
    ...
    int ch_start;            /* Starting channel */
    int ch_cnt;              /* Number of channels */
    int lun_start;           /* Starting LUN */
    int lun_cnt;             /* Number of LUNs */
} hyssd_line_info;
...
typedef struct hyparams {
    ...
    int chs_per_line;   /* # of channels per line (configurable) */
    int luns_per_line;  /* # of LUNs per line (configurable) */
    ...
    int pgs_per_line;
    int blks_per_line;
    int blks_per_position;  /* # of blocks each (ch,lun) position uses in a line */
    int tt_lines;

    int parallel_groups;    /* (nchs/chs_per_line) * (luns_per_ch/luns_per_line) */
    int lines_per_zone;     /* # of lines per zone = parallel_groups */
    ...
    int tt_rpgs;      /* total # of random zone pages (current) */
    int max_rpgs;     /* max # of random zone pages (maptbl/rmap alloc size) */
} hyparams;
