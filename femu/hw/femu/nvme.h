...
/* HYZNS (femu_mode=5) runtime geometry. Kept separate from bb_params/sp
 * so the hyzns/ directory stays self-contained (its own copy).
 * The NAND parallelism (nchs x luns_per_ch) drives
 * the timing model's per-(ch,lun) striping; zone_size_mb sets the logical
 * zone size the host FTL (dm-hyzns) and F2FS/ZenFS see. A "line" (the
 ...
 * zone. See dm-hyzns/dm-hyzns.c. */
typedef struct HyZNSCtrlParams {
...
enum {
    ...
    FEMU_NOSSD_MODE     = 2,
    FEMU_ZNSSD_MODE     = 3,
    FEMU_HYZNS_MODE = 5,    /* host-managed hybrid: R-region (RAN) + S-region (ZNS) */
    FEMU_SMARTSSD_MODE,
    FEMU_KVSSD_MODE,
};
...
static inline bool HYZNS(FemuCtrl *n)
{
    return (n->femu_mode == FEMU_HYZNS_MODE);
}
