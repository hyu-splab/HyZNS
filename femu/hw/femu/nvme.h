...
enum NvmeAdminCommands {
    ...
    NVME_ADM_CMD_MON_HYSSD      = 0xc0,
    NVME_ADMIN_CMD_BBSSD        = 0xc1,
    NVME_ADMIN_CMD_ZNSSD        = 0xc2,
    NVME_ADM_CMD_RESET_DEV      = 0xc3,
    DEVICE_TYPE_OPCODE          = 0xD0,
};
...
typedef struct {
    ...
    union {
        ...
        struct {
            ...
            int num_zones;      /* total # of random zone pages */
        } hyssd;
...
typedef struct BbCtrlParams {
    ...
    int luns_per_ch;
    int nchs;
    int chs_per_line;   /* # of channels per line (configurable striping) */
    int luns_per_line;  /* # of LUNs (ways) per line (configurable way allocation) */
...
typedef struct SsdParams {
    ...
    int ch_xfer_lat;  /* channel transfer latency for one page in nanoseconds
                       ...
                       */
    int num_rzones;
    int max_rzones;   /* max R-zones for maptbl pre-allocation (0 = num_zones) */
    int num_op_line;
    int set_time_model;
} SsdParams;

/* HYHOSTSSD (femu_mode=5) runtime geometry. Kept separate from bb_params/sp
 * so the hyhostssd/ directory stays self-contained (its own copy, mirroring
 * the hyssd/ convention). The NAND parallelism (nchs x luns_per_ch) drives
 * the timing model's per-(ch,lun) striping; zone_size_mb sets the logical
 * zone size the host FTL (dm-hyhost) and F2FS/ZenFS see. A "line" (the
 * device erase / host GC unit) is one NAND block per (ch, lun) in parallel
 * = nchs * luns_per_ch * pgs_per_blk * page_bytes; multiple lines make up a
 * zone. See dm-hyhost/dm-hyhost.c. */
typedef struct HyHostCtrlParams {
    int nchs;          /* # of channels */
    int luns_per_ch;   /* # of LUNs (ways) per channel */
    int page_bytes;    /* NAND page size in bytes (ch/lun striping unit) */
    int pgs_per_blk;   /* NAND erase-block size in pages */
    int zone_size_mb;  /* logical zone size in MiB (0 => 1 GiB default) */
} HyHostCtrlParams;

typedef struct ZNSCtrlParams {
    uint16_t register_model;
    uint32_t num_ch;
    uint32_t num_lun;
...
typedef struct FemuCtrl {
    ...
    uint32_t            old_rzone;
    SsdParams           sp;
    uint64_t            zns_read_cnt;
    uint64_t            zns_read_lat;
    uint64_t            zns_write_cnt;
    uint64_t            zns_write_lat;
    uint64_t            cns_read_cnt;
    uint64_t            cns_read_lat;
    uint64_t            cns_write_cnt;
    uint64_t            cns_write_lat;
    ...
    HyHostCtrlParams hyhost_params;
    ...
    bool            *should_isr;
    bool            poller_on;
    /* Poller quiesce handshake (per-poller, 1-based). A poller sets
     * poller_in_sweep[idx]=true, smp_mb(), then re-checks dataplane_started;
     * nvme_clear_ctrl() clears dataplane_started, smp_mb(), then waits until
     * every poller_in_sweep is false before freeing sq/cq. This is the upstream
     * MoatLab/FEMU fix for the controller-reset vs poller use-after-free
     * (a poller mid-sweep touching freed io_req / dbbuf shadow). The earlier
     * local "poller_busy" attempt lacked the barriers and did not hold. */
...
enum {
    ...
    FEMU_ZNSSD_MODE     = 3,
    FEMU_HYSSD_MODE     = 4,
    FEMU_HYHOSTSSD_MODE = 5,    /* host-managed hybrid: R-region (RAN) + S-region (ZNS) */
    FEMU_SMARTSSD_MODE,
    FEMU_KVSSD_MODE,
};
...
static inline bool HYSSD(FemuCtrl *n)
{
    return (n->femu_mode == FEMU_HYSSD_MODE);
}

static inline bool HYHOSTSSD(FemuCtrl *n)
{
    return (n->femu_mode == FEMU_HYHOSTSSD_MODE);
}
...
#define femu_log(fmt, ...) \
    do { fprintf(stdout, "[FEMU] Log: " fmt, ## __VA_ARGS__); } while (0)
