#!/bin/bash
# sweep_hyztree.sh — Hy-ZTree thread sweep over the FEMU HyZNS device.
# Hy-ZTree keeps internal nodes on a CNS area (F2FS over the dm-hyzns R-region)
# and spills leaves to the ZNS S-region; hyznsd grows the CNS on demand via the
# ctree 2-phase handshake (coord=ctree). Per thread count we (re)build the stack:
#   dm-hyzns(R=INIT_R zones, cap MAX_R) -> mkfs.f2fs -H -A -P -> mount ->
#   hyznsd(coord=ctree) -> run hy-ztree against /dev/mapper/hyzns0.
#
...
#
# hyznsd uses its built-in FTR policy defaults (grow/shrink thresholds); we pass
...
[ -x "$HYD" ]  || { echo "missing hyznsd: $HYD (cd hyznsd && make)"; exit 1; }
...
for NT in $THREADS; do
  ...
  echo "0 $SIZE hyzns $DEV $((INIT_R*ZSEC)) $((MAX_R*ZSEC))" | sudo dmsetup create "$DM"
  ...
  # hyznsd: default FTR policy; only coord=ctree + paths + R clamp are passed.
  sudo "$HYD" -v --set fs=f2fs --set coord=ctree --set dm="$DM" --set backing="$DEV" \
     ...
     > "$OUTDIR/t${NT}_hyznsd.log" 2>&1 &
  sleep 1
  echo "  setup OK: dm(R=$INIT_R,cap$MAX_R) + F2FS($MNT) + hyznsd(coord=ctree)"
