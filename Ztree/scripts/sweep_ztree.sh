#!/bin/bash
# sweep_ztree.sh — ZTree (ZNS-only) thread sweep over the FEMU HyZNS device.
# ZTree has no CNS/F2FS/hyznsd: we create the dm-hyzns target with r_end=0 so
...
for NT in $THREADS; do
  ...
  # dm-hyzns with r_end=0 -> every zone is S (ZNS). No F2FS, no hyznsd.
  ...
  echo "0 $SIZE hyzns $DEV 0 $((MAX_R*ZSEC))" | sudo dmsetup create "$DM"
