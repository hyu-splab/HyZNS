...
/* ── TEMP low-id NLT op trace (set NLT_TRACE 1 to enable; revert after) ──── */
#define NLT_TRACE 0
#define NLT_TRACE_MAXID 256u
#if NLT_TRACE
static inline unsigned nlt_tid(void) { return (unsigned)((uintptr_t)pthread_self() >> 8) % 1000u; }
#endif
                                  ...
                                  ztree_node_id_t node_id)
{
    ...
    for (size_t i = 0; i < cap; i++)
    {
        ...
        {
#if NLT_TRACE
            if (node_id <= NLT_TRACE_MAXID)
                fprintf(stderr, "[NLT t%u] TOMB nid=%u zone=%u => NOTFOUND(empty@i=%zu)\n",
                        nlt_tid(), node_id,
                        atomic_load_explicit(&zone->zone_id, memory_order_relaxed), i);
#endif
            return;                              /* not present */
        }
        if (v == NLT_SLOT_TOMBSTONE)
...
#if NLT_TRACE
                if (node_id <= NLT_TRACE_MAXID)
                    fprintf(stderr, "[NLT t%u] TOMB nid=%u zone=%u => OK(pos=%zu)\n",
                            nlt_tid(), node_id,
                            atomic_load_explicit(&zone->zone_id, memory_order_relaxed), pos);
#endif
                return;
            }
...
#if NLT_TRACE
    if (new_entry->node_id <= NLT_TRACE_MAXID)
        fprintf(stderr, "[NLT t%u] MIGRATE nid=%u prev=%u -> new=%u slot=%u\n",
                nlt_tid(), new_entry->node_id, prev_zone,
                new_entry->zone_id, new_entry->slot_id);
#endif
