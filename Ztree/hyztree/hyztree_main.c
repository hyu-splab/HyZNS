...
/* daemon-driven grow (Style-1, 2-phase handshake): the hyznsd daemon owns the
...
/* hyzns F2FS variant: CNS lives on an F2FS mount over the dm-hyzns
...
/* When F2FS sits on a dm-hyzns target, the real ENOSPC pressure is dm's
...
cow_tree *cow_open(const char *path)
{
     ...
     * Override with env CNS_ODIRECT=1 for O_DIRECT (no kernel cache). */
    {
        ...
        fprintf(stderr, "[ctree_hyzns_f2fs] CNS reclaim: %s\n",
        ...
        fprintf(stderr, "[ctree_hyzns_f2fs] CNS backpressure cap: %llu ms\n",
    ...
    for (int k = 0; k < CTREE_CNS_SHARDS; k++)
    {
        ...
        {
            fprintf(stderr,
                    "[ctree_hyzns_f2fs] FATAL: cannot open CNS file %s: %s\n"
                    "  Ensure F2FS is mounted on %s (dm-hyzns R-region):\n"
                    "    sudo ~/HYSSD/dm-hyzns/scripts/f2fs_setup.sh /dev/nvme0n1 <r_end> %s\n",
    ...
    fprintf(stderr, "[ctree_hyzns_f2fs] CNS mount: %s (%s)\n",
            cns_dir(), cns_fstype_name(cns_dir()));
    fprintf(stderr,
            "[ctree_hyzns_f2fs] CNS mode: %s (%d shards in %s)\n",
    ...
    /* hyzns: the device's front zones are the R-region (CNS, owned by
     * dm-hyzns + F2FS).  ctree's ZNS structures (meta z0/1, LLayer z2+) must
     ...
     * reserved==0 (real full-ZNS namespace) makes this a no-op. */
    {
        ...
        if (reserved > 0 && reserved < t->info.nr_zones)
        {
            ...
            fprintf(stderr,
                    "[ctree_hyzns_f2fs] reserved(R)=%u zones skipped; ZNS uses %u "
                    "S-zones (logical 0 = phys zone %u)\n",
                    reserved, t->info.nr_zones, reserved);
        }
        else
        {
            fprintf(stderr, "[ctree_hyzns_f2fs] reserved(R)=%u; no zone offset "
    ...
    {
        /* Daemon-driven grow (HyZNS §5.6.3).  The hyznsd daemon owns the grow
