...
Two Copy-on-Write B+-tree variants for the HYHOSTSSD, compared in paper Fig.13:
...
- **Hy-ZTree** (`hyztree/`) — hybrid. Internal nodes live on a CNS area (F2FS
  over the dm-hyhost R-region) while leaves are written to the ZNS S-region and
  spill to CNS under ZNS lock contention. `hyhostd` grows the CNS on demand
...
Both sweeps run on the emulated device through the dm-hyhost target
(`/dev/mapper/hyhost0`). ZTree sets `r_end=0` (whole device is one ZNS region);
Hy-ZTree builds the dm + F2FS + hyhostd stack per thread count. Hy-ZTree needs
the `hyhostd` daemon and a patched `f2fs_io` (`resize_cns`) — see the top-level
`experiments/README.md` build step.

`hyhostd` is launched with its built-in FTR policy defaults; the sweep passes
