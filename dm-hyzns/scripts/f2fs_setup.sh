#!/bin/bash
#
# Bring up F2FS on top of a dm-hyzns target sized to its R-region.
# This exercises the dm-hyzns path with a real filesystem; no ZenFS,
# no zoned namespace, no S-region access.
#
# Usage:
#   sudo ./scripts/f2fs_setup.sh [BACKING] [R_END_LBA] [MNT]
#
# Defaults: BACKING=/dev/nvme0n1  R_END_LBA=16777216 (8 GiB)  MNT=/mnt/hyzns
#
# What it does:
#   1. (re)load dm_hyzns
...
echo "0 ${R_END} hyzns ${BACKING} ${R_END}" | dmsetup create "${NAME}"
