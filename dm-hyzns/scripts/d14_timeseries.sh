...
# Snapshot config + initial status into meta.
{
    ...
    echo "status_initial=$(dmsetup status "$NAME" | sed 's/^.*hyzns //')"
} > "$META"
...
(
    ...
    while true; do
        now=$(date +%s)
        elapsed=$((now - start))
        line=$(dmsetup status "$NAME" 2>/dev/null | sed 's/^.*hyzns //') || break
        echo "$elapsed $line"
        sleep 1
    done
) > "$DM_LOG" &
...

{
    ...
    echo "status_final=$(dmsetup status "$NAME" | sed 's/^.*hyzns //')"
} >> "$META"
