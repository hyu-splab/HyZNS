...
set -uo pipefail
cd "$(dirname "$0")/.."                                  # hyhostd/
RDB=../rocksdb; ZENFS=$RDB/plugin/zenfs/util/zenfs
source ../dm-hyhost/scripts/_lib.sh
BACKING=/dev/nvme0n1; NAME=hyhost0; DEV=/dev/mapper/$NAME; AUX=/mnt/aux
ZS=$HYHOST_ZONE_SECTORS; F2FS_IO=${F2FS_IO:-../f2fs-tools-1.16.0/tools/f2fs_io/f2fs_io}
...
for b in "$ZENFS" "$BIN" ./hyhostd; do [ -x "$b" ]||{ echo "missing $b"; exit 1; }; done
...
umount "$AUX" 2>/dev/null||true; dmsetup remove "$NAME" 2>/dev/null||true
../dm-hyhost/scripts/load.sh>/dev/null   # ALWAYS reload (rmmod+insmod) -> run uses the freshly-built .ko
reset_device_full_r "$BACKING">/dev/null 2>&1||true
if [[ $MAXPROV == 1 ]]; then
  echo "0 $SZ hyhost $BACKING $((R_INIT*ZS)) $((R_MAX*ZS))"|dmsetup create "$NAME"||exit 1   # 6th=max -> R can grow
  dmsetup message "$NAME" 0 set_r_end $((R_INIT*ZS))||exit 1
  mkfs.f2fs -f -m -H -A -P "$DEV">/dev/null 2>&1||{ echo mkfs fail; exit 1; }
else
  echo "0 $SZ hyhost $BACKING $((R_INIT*ZS))"|dmsetup create "$NAME"||exit 1                  # 5th only -> R fixed at R_INIT
  dmsetup message "$NAME" 0 set_r_end $((R_INIT*ZS))||exit 1
...
# daemon. Policy flags come from hyhostd.conf (single source of truth); an env
...
if [[ $DAEMON == 1 ]]; then
  ...
  echo "RMAX=$R_MAX"
  ./hyhostd -q -c hyhostd.conf --set dm=$NAME --set backing=$BACKING --set fs=zenfs --set aux="$AUX/i1" \
