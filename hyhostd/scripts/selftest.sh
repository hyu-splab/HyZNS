#!/bin/bash
# selftest.sh — offline validation of hyhostd policy with mock device/report
...
#
# The mock emits the current dm-hyhost status (line_pblocks, zone_pblocks,
...
echo "hyhostd offline policy self-test (line/zone unit handling + policy decisions)"
...
oldZS=2097152
printf '0 536870912 hyhost r_end=%d pages=0 lines=0 line_pblocks=262144 block_pages=512 gc_policy=greedy free_lines=1 dirty=0 cur_off=0 nospc=0 valid_pages=0 valid_lines=0 free_pages=262144 gc_blocked=0\n' \
