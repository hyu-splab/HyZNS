...
set -uo pipefail
source /data/HYSSD/dm-hyhost/scripts/_lib.sh
HERE=$(cd "$(dirname "$0")" && pwd)
B=/dev/nvme0n1; N=hyhost0; DEV=/dev/mapper/$N; MNT=/mnt/aux; AUXI=$MNT/i1
# use the installed f2fs_io (the source-tree binary may not be built here);
# env-overridable.
F2IO=${F2IO:-/usr/local/sbin/f2fs_io}
ZENFS=/data/HYSSD/rocksdb/plugin/zenfs/util/zenfs
BIN=${BIN:-/data/HYSSD/rocksdb/db_bench_zns}   # standard build: WAL+L0 on S
ZS=$HYHOST_ZONE_SECTORS; SZ=$(blockdev --getsz "$B"); SLAST=$(( SZ/ZS - 1 ))
...
RES=/data/HYSSD/hyhostd/results/resize_ops
RCAP=$(( SZ/ZS - 8 ))
...
/data/HYSSD/dm-hyhost/scripts/load.sh >/dev/null 2>&1
reset_device_full_r "$B" >/dev/null 2>&1||true
echo "0 $SZ hyhost $B $((RMAX*ZS)) $SZ" | dmsetup create "$N"; dmsetup message "$N" 0 set_r_end $((RMAX*ZS))
