/* hyhostd.c - main: config, monitor loop, cooldown/backoff, logging.
 * See DESIGN.md and hyhostd.h for the contract. */
#define _GNU_SOURCE
#include "hyhostd.h"
#include <stdio.h>
#include <stdlib.h>
...
{
	fprintf(stderr,
"hyhostd " HYHOSTD_VERSION " - HyZNS CNS-area autoscaling daemon\n"
...
int main(int argc, char **argv)
{
	...
	logline("hyhostd %s start: dm=%s fs=%s rz=%s mnt=%s aux=%s poll=%ums%s%s",
	        HYHOSTD_VERSION, cfg.dm, cfg.fs == FS_ZENFS ? "zenfs" : "f2fs",
	...
	do {
		...
		if (dm_rc != 0) {
			 ...
			 * the mounted block device name (dm-hyhost == dm-0 in our setup;
