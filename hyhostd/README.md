...
from `dm-hyhost`) and the ZNS area (**S**, from the zone-aware FS or
...
(= 16 dm-hyhost lines at the current geometry).
...
| File | Role |
|---|---|
| `hyhostd.h` | contract constants, config/state/decision types, prototypes |
| `hyhostd.c` | main: config parse, monitor loop, cooldown/backoff, logging, util |
...
| `hyhostd.conf.example` | annotated config |
...
- **Resize trigger** (absolute target R):
  - F2FS: `f2fs_io resize_cns <target> <mnt>/.hyhostd_probe` (kernel orchestrates
...
- Paper defaults 2/5/3/4; thresholds are administrator-configurable in
  `hyhostd.conf`. Exponential backoff on resize failure and a post-resize
  cooldown are enforced by the main loop.

## Deploy as a systemd daemon
`hyhostd` is daemon-shaped (continuous loop, SIGTERM clean-shutdown, config
...
```sh
sudo make install        # -> /usr/local/sbin/hyhostd, /etc/systemd/system/hyhostd.service,
                         #    /etc/hyhostd.conf (kept if present)
sudo systemctl daemon-reload
sudo systemctl enable --now hyhostd
journalctl -u hyhostd -f          # watch decisions/grows live
```
`Restart=on-failure` makes a device hiccup self-healing (the daemon re-derives
state on restart); `ConditionPathExists=/dev/mapper/hyhost0` keeps it from starting
before the device exists. Files: `systemd/hyhostd.service`, `systemd/hyhostd.conf`.
...
# daemon
sudo ./hyhostd -c hyhostd.conf            # continuous
sudo ./hyhostd -c hyhostd.conf --dry-run  # decide + log, no act
sudo ./hyhostd --once -v --set dm=hyhost0 --set fs=f2fs --set mnt=/mnt/hyhost
```
...
sudo ./hyhostd --set fs=zenfs --set rz_source=fs \
  --set aux=/mnt/zenfs_aux --set dm=hyhost0
```
