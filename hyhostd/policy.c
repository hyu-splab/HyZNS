 ...
 * Paper defaults 2/5/3/4, administrator-configurable via hyhostd.conf
...
hyd_decision hyd_decide(const hyd_cfg *cfg, const hyd_state *st)
{
	...
	/* gc_urgent: two independently-switchable sources (hyhostd.conf).
...
static int act_f2fs(const hyd_cfg *cfg, uint32_t target, char *resmsg, size_t n)
{
	...
	char probe[512];
	snprintf(probe, sizeof probe, "%s/.hyhostd_probe", cfg->mnt);
