...
/* Get zone size in F2FS blocks from the block device backing the file */
static unsigned int get_zone_blocks(int fd)
{
	struct stat st;
	char path[PATH_MAX];
	FILE *fp;
	...
	snprintf(path, sizeof(path),
		 "/sys/dev/block/%d:%d/queue/chunk_sectors",
	...
	if (fscanf(fp, "%u", &chunk_sectors) != 1)
		chunk_sectors = 0;
	fclose(fp);

	/* Convert 512-byte sectors to F2FS blocks (4KB) */
	return chunk_sectors / (F2FS_BLKSIZE / 512);
}

#define gc_force_desc "trigger filesystem gc_force for resize_cns"
#define gc_force_help "f2fs_io gc_force [start zoneID] [# of zone (>0)] [file_path]\n\n"

static void do_gc_force(int argc, char **argv, const struct cmd_desc *cmd)
{
	struct f2fs_gc_range range;
	int ret, fd;

	u64 id_zone, num_zone, range_zone;
	...
	id_zone = (u64)atoi(argv[1]);
	num_zone = (u64)atoi(argv[2]) - 1;
	range_zone = id_zone + num_zone;

	fd = xopen(argv[3], O_RDWR, 0);

	unsigned int zone_blocks = get_zone_blocks(fd);
	if (!zone_blocks)
		die("Failed to get zone size from device");

	range.sync = 1;
	range.start = id_zone * zone_blocks;
	range.len = num_zone * zone_blocks;
	range.force = true;

	printf("sync : %s start : %ld len : %ld gc trigger %s (zone_blocks=%u)\n",
				range.sync ? "synchronous" : "asynchronous",
				range.start, range.len, range.force ? "force" : "!force",
				zone_blocks);

	ret = ioctl(fd, F2FS_IOC_GARBAGE_COLLECT_FORCE, &range);
	if (ret < 0) {
		die_errno("F2FS_IOC_GARBAGE_COLLECT_RANGE (force) failed");
	}

	printf("trigger %s gc_force : ZoneID[%"PRIu64" - %"PRIu64"] ret=%d\n",
		range.sync ? "synchronous" : "asynchronous",
		id_zone, range_zone, ret);
	exit(0);
}

#define resize_cns_desc "Modify R zone"
#define resize_cns_help "f2fs_io resize_cns [# of new_rzone] [file_path]\n\n"

static void do_resize_cns(int argc, char **argv, const struct cmd_desc *cmd)
{
	u32 new_rzone;
	int ret, fd;
	...
	new_rzone = (u32)atoi(argv[1]);
	fd = xopen(argv[2], O_RDWR, 0);

	ret = ioctl(fd, F2FS_IOC_RESIZE_CNS, &new_rzone);
	if (ret < 0) {
		die_errno("F2FS_IOC_MODIFY_FS failed");
	}

	printf("Success: The 'modify cmd' request was completed successfully\n");

	exit(0);
}

#define gc_n_modify_desc "GC Force and Modify R zone"
#define gc_n_modify_help "f2fs_io gc_n_modify [start zoneID] [# of zone (>0)] [file_path]\n\n"\
"\t\t\t : new_rzone = [# of zone] + old_zone\n\n"\

static void do_gc_n_modify(int argc, char **argv, const struct cmd_desc *cmd)
{
	struct f2fs_gc_range range;
	int ret, fd;
	...
	unsigned int zone_blocks_m = get_zone_blocks(fd);
	if (!zone_blocks_m)
		die("Failed to get zone size from device");

	range.sync = 1;
	range.start = id_zone * zone_blocks_m;
	range.len = num_zone * zone_blocks_m;
	range.force = true;
	...
	ret = ioctl(fd, F2FS_IOC_GC_FORCE_n_MODIFY, &range);
	if (ret < 0) {
		die_errno("F2FS_IOC_GC_FORCE_n_MODIFY failed");
	}
...
#define print_cnt_discard_desc "Print cnt of discard CMD"
#define print_cnt_discard_help "f2fs_io print_cnt_discard [file]\n\n"

static void do_print_cnt_discard(int argc, char **argv, const struct cmd_desc *cmd)
{
	struct f2fs_cnt_cmd discard;
	int ret, fd, i;
	int heat_level_counts[4];
	...
	ret = ioctl(fd, F2FS_IOC_PRINT_CNT_DISCARD, &discard);
	if (ret < 0)
		die_errno("F2FS_IOC_PRINT_CNT_DISCARD failed");

	printf("call_discard_cmd_bbssd : %lu call_discard_cmd_znssd : %lu\n", 
				discard.call_discard_cmd_bbssd, 
				discard.call_discard_cmd_znssd);
	
	heat_level_counts[HOT] = discard.hot;
	heat_level_counts[WARM] = discard.warm;
	heat_level_counts[COLD] = discard.cold;
	heat_level_counts[NR_TEMP_TYPE] = discard.cold_gc;

	printf("=== Heat Level Distribution ===\n");
	for (i = 0; i<NR_TEMP_TYPE+1; i++) {
		printf("%-10s : %d\n", heat_level_names[i], heat_level_counts[i]);
	}
	printf("===============================\n");
...
#define reset_cnt_discard_desc "Reset cnt of discard CMD"
#define reset_cnt_discard_help "f2fs_io reset_cnt_discard [file]\n\n"

static void do_reset_cnt_discard(int argc, char **argv, const struct cmd_desc *cmd)
{
	int ret, fd;
	...
	ret = ioctl(fd, F2FS_IOC_RESET_CNT_DISCARD);
	if (ret < 0)
		die_errno("F2FS_IOC_PRINT_CNT_DISCARD failed");

	printf("init cnt discard succeess\n");
...
const struct cmd_desc cmd_list[] = {
	...
	CMD(resize_cns),
	CMD(gc_n_modify),
	CMD(print_cnt_discard),
	CMD(reset_cnt_discard),
	CMD(listxattr),
	CMD(setxattr),
	CMD(removexattr),
