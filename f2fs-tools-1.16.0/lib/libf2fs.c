...
#define MOUNTED _PATH_MOUNTED
	...
	if (stat(path, st_buf) == 0 && S_ISBLK(st_buf->st_mode)) {
		MSG(0, "Info: Device path : %s\n", path);
		int fd = open(path, O_RDONLY | O_EXCL);
		MSG(0, "Info: Device fd : %d\n", fd);
...
int get_device_info(int i)
{
	...
	if (dev->zoned_model != F2FS_ZONED_NONE) {
		...
		/*
		 * For HYSSD mode: if num_Rzone is not set via -B option,
		 * automatically detect it from the device BEFORE checking zones
		 */
		if (c.hyssd_mode && c.num_Rzone == 0) {
			if (f2fs_get_nr_rnd_zones(i) < 0) {
				MSG(0, "\tError: Failed to get number of random writable zones\n");
				free(stat_buf);
				return -1;
			}
		}
		...
		if (c.conf_reserved_sections) {
			...
			if (!c.overprovision && c.conf_reserved_sections_zns==0) {
	...
	if (c.zenfs_aux_mode && c.zenfs_aux_maxprov && i == 0) {
		/* ZenFS aux max-provision (-A -P): size the F2FS volume (and thus
		 * SIT/NAT/SSA + segment_count_main) for the WHOLE device, so the
		 * kernel's in-memory structures (built from MAIN_SEGS at mount)
		 * can later grow the usable R up to the device limit. Only the
		 * current R is usable now; the kernel gates the usable boundary
		 * (MAIN_SECS) down to the current ABA at mount via
		 * f2fs_aux_gate_init(). Metadata sits at the front (R-region);
		 * mkfs writes only there. */
		dev->total_sectors = dev->nr_zones * dev->zone_blocks * c.sectors_per_blk;
		MSG(0, "Info: ZenFS aux max-provision - F2FS metadata sized for whole device (max R)\n");
		MSG(0, "      dev[0]->total sectors : %ld (max-provision; usable gated to ABA at mount)\n", dev->total_sectors);
	} else if (c.zenfs_aux_mode && i == 0) {
		/* ZenFS aux mode: F2FS uses only rzone (conventional zones) */
		dev->total_sectors = dev->nr_rnd_zones * dev->zone_blocks * c.sectors_per_blk;
		MSG(0, "Info: ZenFS aux mode - F2FS uses only rzone\n");
		MSG(0, "      dev[0]->total sectors : %ld (rzone only)\n", dev->total_sectors);
	} else if (c.zenfs_aux_mode && i == 1) {
		/* ZenFS aux mode: dev[1] (szone) is for ZenFS, not F2FS */
		dev->total_sectors = (dev->nr_zones - dev->nr_rnd_zones) * dev->zone_blocks * c.sectors_per_blk;
		MSG(0, "      dev[1]->total sectors : %ld (ZenFS only, F2FS ignores)\n", dev->total_sectors);
		/* Do NOT add to c.total_sectors - F2FS should not use this space */
		free(stat_buf);
		return 0;  /* Early return to skip c.total_sectors += */
	} else if (c.hyssd_mode && i == 0) {
		dev->total_sectors = dev->nr_rnd_zones * dev->zone_blocks * c.sectors_per_blk;
		MSG(0, "dev[0]->total sectors : %ld\n", dev->total_sectors);
	} else if (c.hyssd_mode && i == 1) {
		dev->total_sectors = (dev->nr_zones - dev->nr_rnd_zones) * dev->zone_blocks * c.sectors_per_blk;
		MSG(0, "dev[1]->total sectors : %ld\n", dev->total_sectors);
	}
...
int f2fs_get_f2fs_info(void)
{
	...
	if (c.zoned_model != F2FS_ZONED_NONE) {
		 ...
		 */
		if (c.zenfs_aux_mode) {
			/* ZenFS aux mode: Use conventional SSD settings (like BBSSD)
			 * since F2FS only uses small rzone area, not full zoned device */
			c.segs_per_sec = 1;
			c.secs_per_zone = 1;
			MSG(0, "Info: ZenFS aux mode - using segs_per_sec=1 (BBSSD-like)\n");
		} else {
