...
int f2fs_get_nr_rnd_zones(int i)
{
	struct device_info *dev = c.devices + i;
	struct blk_rzone_report rep;
	int ret;

	if (!c.hyssd_mode) {
		MSG(0, "\tInfo: Not in HYSSD mode, skipping rzone detection\n");
		return 0;
	}

	/* Call BLKREPORTABA ioctl to get number of random writable zones */
	ret = ioctl(dev->fd, BLKREPORTABA, &rep);
	if (ret != 0) {
		ret = -errno;
		ERR_MSG("ioctl BLKREPORTABA failed: errno=%d\n", errno);
		return ret;
	}

	/* The data field contains the number of random writable zones */
	c.num_Rzone = rep.data;

	MSG(0, "Info: Detected %u random writable zones (Rzones) from device\n",
	    c.num_Rzone);
...
int f2fs_check_zones(int j)
{
	...
	while (sector < total_sectors) {
		...
		// the device reports every zone as seq-write type;
		// with -B the first num_Rzone zones are treated as conventional
		blkz = (struct blk_zone *)(rep + 1);
		int rnd_zone_idx = 0;
		for (i = 0; i < rep->nr_zones && sector < total_sectors; i++) {
			...
			if (blk_zone_conv(blkz) ||
			    blk_zone_seq_pref(blkz) || (c.hyssd_mode && rnd_zone_idx < c.num_Rzone)) {
				if (last_is_conv)
					dev->nr_rnd_zones++;
			} else {
				last_is_conv = 0;
			...
			if (blk_zone_conv(blkz) || (c.hyssd_mode && rnd_zone_idx < c.num_Rzone)) {
				DBG(2,
