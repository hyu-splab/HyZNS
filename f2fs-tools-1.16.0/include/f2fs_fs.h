...
struct f2fs_configuration {
	...
	uint32_t conf_reserved_sections_bb; // for only BBSSD (fair vs. multi dev op)
	uint32_t conf_reserved_sections_zns; // (fair vs. bb dev op)
	uint32_t reserved_segments;
	uint32_t new_reserved_segments;
	int sparse_mode;
	int hyssd_mode; //for HYSSD
	int num_Rzone; //for HYSSD
	int zenfs_aux_mode; //for ZenFS aux_path (F2FS uses only rzone)
	int zenfs_aux_maxprov; //ZenFS aux max-provision: size metadata for whole device (max R); kernel gates usable area to current ABA at mount (-A -P)
	int single_zns_mode; // for only ZNSSD + F2FS (not used)
	int zoned_mode;
	int zoned_model;
	size_t zone_blocks;
...
extern int f2fs_get_nr_rnd_zones(int);
...
static inline double get_reserved(struct f2fs_super_block *sb, double ovp)
{
	...
	if (c.conf_reserved_sections_zns){
		reserved = c.conf_reserved_sections_zns;
		return reserved;
	}

	if (c.conf_reserved_sections_bb){
		reserved = c.conf_reserved_sections_bb * segs_per_sec;
