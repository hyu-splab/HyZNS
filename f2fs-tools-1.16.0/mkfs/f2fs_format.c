...
static int f2fs_prepare_super_block(void)
{
	...
	for (i = 0; i < c.ndevs; i++) {
		if(c.zenfs_aux_mode){
			/*
			 * ZenFS aux mode: F2FS uses only rzone (dev[0])
			 * dev[1] (szone) is managed by ZenFS, so we set its segments to 0
			 */
			if (i == 0) {
				/* dev[0]: rzone for F2FS */
				...
				c.devices[i].end_blkaddr = c.devices[i].total_segments * c.blks_per_seg - 1 + sb->segment0_blkaddr;
			} else {
				/* dev[1]: szone for ZenFS - F2FS should not use this */
				c.devices[i].total_segments = 0;  /* Set to 0 so F2FS ignores it */
				c.devices[i].start_blkaddr = c.devices[i - 1].end_blkaddr + 1;
				c.devices[i].end_blkaddr = c.devices[i].start_blkaddr - 1;  /* Invalid range */
			}
		}
		else if(c.hyssd_mode){
			if (i==0){
				/* dev[0]: rzone (conventional) */
				c.devices[i].total_segments = (c.devices[i].total_sectors * c.sector_size - zone_align_start_offset) / segment_size_bytes;
				c.devices[i].start_blkaddr = 0;
				/* Use dynamic calculation for flexibility when rzone ratio changes */
				c.devices[i].end_blkaddr = c.devices[i].total_segments * c.blks_per_seg - 1 + sb->segment0_blkaddr;
			}
			else {
				/* dev[1]: szone (sequential) */
				c.devices[i].total_segments = c.devices[i].total_sectors / (c.sectors_per_blk * c.blks_per_seg);
				c.devices[i].start_blkaddr = c.devices[i - 1].end_blkaddr + 1;
				c.devices[i].end_blkaddr = c.devices[i].start_blkaddr + c.devices[i].total_segments *
											c.blks_per_seg - 1;
			}
		}
		else{
		...
		printf("***********************\n");
		printf("c.devices[%d].total_sectors : %ld \n", i, c.devices[i].total_sectors);
		printf("c.devices[%d].total_segments : %d \n", i, c.devices[i].total_segments);
		printf("c.devices[%d].start_blkaddr : %ld \n", i, c.devices[i].start_blkaddr);
		printf("c.devices[%d].end_blkaddr : %ld \n", i, c.devices[i].end_blkaddr);
		printf("***********************\n");
		if (c.ndevs > 1) {
			/*
			 * ZenFS aux mode: don't write dev[1] to superblock.
			 * F2FS only uses rzone (dev[0]). If dev[1] path is
			 * in the superblock, the kernel will open it and fail
			 * the BLK_ZONED_HM check when BLKZONED is not set.
			 */
			if (c.zenfs_aux_mode && i > 0)
				continue;
	...
	if (c.overprovision == 0) {
		...
		/* For ZenFS aux mode with small rzone, ensure minimum overprovision */
		if (c.zenfs_aux_mode && c.overprovision == 0) {
			c.overprovision = 5.0;  /* Use 5% as default for small aux device */
			MSG(0, "Info: ZenFS aux mode - setting overprovision to %.1lf%%\n",
			    c.overprovision);
		}
	}
	...
	MSG(0, "Debug: c.overprovision = %.3lf%%\n", c.overprovision);
	MSG(0, "Debug: c.total_segments = %u, F2FS_MIN_SEGMENTS = %d\n",
	    c.total_segments, F2FS_MIN_SEGMENTS);
	MSG(0, "Debug: segment_count_main = %u, NR_CURSEG_TYPE = %d\n",
	    get_sb(segment_count_main), NR_CURSEG_TYPE);
	MSG(0, "Debug: c.reserved_segments = %u\n", c.reserved_segments);
	MSG(0, "Debug: segment_count_main - NR_CURSEG_TYPE = %u\n",
	    get_sb(segment_count_main) - NR_CURSEG_TYPE);
	MSG(0, "Debug: c.feature = 0x%x, F2FS_FEATURE_RO = 0x%x\n",
	    c.feature, F2FS_FEATURE_RO);
	...
	if ((!(c.feature & F2FS_FEATURE_RO) &&
		...
		(get_sb(segment_count_main) - NR_CURSEG_TYPE) <
						c.reserved_segments) {
		MSG(0, "Debug: Validation failed! Going to too_small\n");
		goto too_small;
	}
	...
	} else if (c.zenfs_aux_maxprov) {
		/*
		 * aux max-provision (-A -P): segment_count_main spans the WHOLE
		 * device so the kernel mount-time gate can re-expose tail
		 * sections when the ABA grows. The default layout spreads
		 * COLD_DATA / WARM_DATA to total_zones>>2 / >>1 (here ~16320 /
		 * ~32641 for a 128 GiB max-prov), which lands them deep in the
		 * S-region. When the kernel gates MAIN_SEGS down to the current
		 * ABA at mount, those stranded cursegs become invalid (invalid
		 * blkaddr + SIT-flush BUG). So cluster all six cursegs in the
		 * low (initial-ABA) zones; the gate-down then never strands one.
		 * NB: this requires the initial ABA to be >= 6 zones.
		 */
...
static int f2fs_init_sit_area(void)
{
	...
	fprintf(stderr, "\tFilling sit area at offset 0x%08"PRIx64"\n", sit_seg_addr);
...
static int f2fs_init_nat_area(void)
{
	...
	fprintf(stderr, "\tFilling nat area at offset 0x%08"PRIx64"\n", nat_seg_addr);
...
static int f2fs_write_check_point_pack(void)
{
	...
	if (c.conf_reserved_sections) {
		...
		if (get_cp(overprov_segment_count) < 2 * get_sb(segs_per_sec)) {
			MSG(0, "\tError: Not enough overprovision segments (%u) < 2 *  get_sb(segs_per_sec) (%u)\n",
			    get_cp(overprov_segment_count),  2 * get_sb(segs_per_sec));
		...
		if(c.conf_reserved_sections_zns){
			set_cp(overprov_segment_count, c.overprovision);
		}
	 }
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	DBG(1, "\tWriting main segments, cp at offset 0x%08"PRIx64"\n",
						cp_seg_blk);
	fprintf(stderr, "\tWriting main segments, cp at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	*/
	...
	if (c.feature & F2FS_FEATURE_RO) {
		...
		for (i = CURSEG_HOT_DATA; i < NR_CURSEG_TYPE; i++) {
			...
			fprintf(stderr, "\t\tjournal->sit_j.entries[%d].segno = %d \n", i, journal->sit_j.entries[i].segno);
			fprintf(stderr, "\t\tvblocks : %d, journal->sit_j.entries[%d].se.vblocks = %d \n", vblocks, i, journal->sit_j.entries[i].se.vblocks);
		}

		journal->n_sits = cpu_to_le16(6);
		fprintf(stderr, "\t\tjournal->n_sits = %d \n", journal->n_sits);
	}
	...
	DBG(1, "\tWriting Segment summary for HOT/WARM/COLD_DATA, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	fprintf(stderr, "\tWriting Segment summary for HOT/WARM/COLD_DATA, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	DBG(1, "\tWriting Segment summary for HOT_NODE, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	fprintf(stderr, "\tWriting Segment summary for HOT_NODE, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	DBG(1, "\tWriting Segment summary for WARM_NODE, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	fprintf(stderr, "\tWriting Segment summary for WARM_NODE, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	DBG(1, "\tWriting Segment summary for COLD_NODE, at offset 0x%08"PRIx64"\n",
			cp_seg_blk);
	fprintf(stderr, "\tWriting Segment summary for COLD_NODE, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	fprintf(stderr, "\tWriting cp page2, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	if (flags & CP_NAT_BITS_FLAG) {
		...
		*(__le64 *)nat_bits = get_cp_crc(cp);
		...
		DBG(1, "\tWriting NAT bits pages, at offset 0x%08"PRIx64"\n",
					cp_seg_blk);
		fprintf(stderr, "\tWriting NAT bits pages, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	DBG(1, "\tWriting cp page 1 of checkpoint pack 2, at offset 0x%08"PRIx64"\n",
				cp_seg_blk);
	fprintf(stderr, "\tWriting cp page 1 of checkpoint pack 2, at offset 0x%08"PRIx64"\n", cp_seg_blk);
	...
	DBG(1, "\tWriting cp page 2 of checkpoint pack 2, at offset 0x%08"PRIx64"\n",
				cp_seg_blk);
	fprintf(stderr, "\tWriting cp page 2 of checkpoint pack 2, at offset 0x%08"PRIx64"\n", cp_seg_blk);
...
static int f2fs_write_super_block(void)
{
	...
	fprintf(stderr, "\tWriting super block, at offset 0x%08x\n", 0);
...
static int f2fs_discard_obsolete_dnode(void)
{
	...
	do {
		...
		fprintf(stderr, "\tDiscard dnode, at offset 0x%08"PRIx64"\n", offset);
...
static block_t f2fs_add_default_dentry_root(void)
{
	...
	fprintf(stderr, "\tWriting default dentry root, at offset 0x%x\n", data_blkaddr);
...
static int f2fs_write_root_inode(void)
{
	...
	fprintf(stderr, "\tWriting root inode (hot node), offset 0x%x\n", node_blkaddr);
...
static int f2fs_write_default_quota(int qtype, __le32 raw_id)
{
	...
	for (i = 0; i < QUOTA_DATA; i++) {
		...
		fprintf(stderr, "\tWriting quota data, at offset %08x (%d/%d)\n", blkaddr, i + 1, QUOTA_DATA);
...
static int f2fs_write_qf_inode(int qtype)
{
	...
	fprintf(stderr, "\tWriting quota inode (hot node), offset 0x%x\n", node_blkaddr);
...
static int f2fs_update_nat_default(void)
{
	...
	DBG(1, "\tWriting nat root, at offset 0x%08"PRIx64"\n",
					nat_seg_blk_offset);
	fprintf(stderr, "\tWriting nat root, at offset 0x%08"PRIx64"\n", nat_seg_blk_offset);
...
static block_t f2fs_add_default_dentry_lpf(void)
{
	...
	DBG(1, "\tWriting default dentry lost+found, at offset 0x%x\n",
							data_blkaddr);
	fprintf(stderr, "\tWriting default dentry lost+found, at offset 0x%x\n", data_blkaddr);
...
static int f2fs_write_lpf_inode(void)
{
	...
	DBG(1, "\tWriting lost+found inode (hot node), offset 0x%x\n",
								node_blkaddr);
	fprintf(stderr, "\tWriting lost+found inode (hot node), offset 0x%x\n", node_blkaddr);
