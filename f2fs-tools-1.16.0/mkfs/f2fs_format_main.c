...
static void mkfs_usage()
{
	...
	MSG(0, "  -A ZenFS aux_path mode: F2FS uses only rzone (requires -H)\n");
	MSG(0, "  -b filesystem block size [default:4096]\n");
	MSG(0, "  -B [# conventional zone] for super block // HYSSD\n");
	...
	MSG(0, "  -H support HYSSD device [default:0]\n");
	MSG(0, "  -i extended node bitmap, node ratio is 20%% by default\n");
	MSG(0, "  -l label\n");
	MSG(0, "  -U uuid\n");
	MSG(0, "  -m support zoned block device [default:0]\n");
	MSG(0, "  -N support single zns ssd device [default:0]\n"); // not used.
	...
	MSG(0, "  -y # of reserved sections [for EX : BBSSD]\n");
	MSG(0, "  -Y set \"static ovp, static reserved_sections\", use with -o [for EX : BBSSD & ZNSSD]\n");
...
{
	static const char *option_string = "qAPa:b:B:c:C:d:e:E:g:hHil:mNo:O:rR:s:S:z:t:T:U:Vfw:y:Y:Z:"; // a trailing ':' means the option takes an argument
	...
	while ((option = getopt_long(argc,argv,option_string,long_opts,NULL)) != EOF) {
		...
		case 'A':
			c.zenfs_aux_mode = 1;
			break;
		case 'P': /* ZenFS aux max-provision (use with -A) */
			c.zenfs_aux_maxprov = 1;
			c.feature |= F2FS_FEATURE_AUX_MAXPROV;
			break;
		case 'a':
			c.heap = atoi(optarg);
			break;
		...
		case 'C':
			...
			break;
		case 'y':
			c.conf_reserved_sections_bb = atoi(optarg);
			break;
		case 'Y':
			c.conf_reserved_sections_zns = atoi(optarg);
			break;
		case 'Z':
	...
	if(c.conf_reserved_sections_zns && !c.overprovision){
		MSG(0, "\tInfo: use together with -o\n");
		exit(1);
	}
	...
	if (c.hyssd_mode) {
		c.devices[c.ndevs++].path = strdup(argv[optind]);
	}
...
int main(int argc, char *argv[])
{
	...
	/* Validate ZenFS aux mode requirements */
	if (c.zenfs_aux_mode && !c.hyssd_mode) {
		MSG(0, "\tError: ZenFS aux mode (-A) requires HYSSD mode (-H)\n");
		mkfs_usage();
	}
	...
	/*
	 * ZenFS aux mode: F2FS only uses rzone (conventional zones).
	 * Zone detection above has already identified rzones and computed
	 * rzone-only total_sectors for dev[0]. Now switch to conventional
	 * mode so F2FS formats without zoned overhead (no large zone-sized
	 * sections, no F2FS_FEATURE_BLKZONED, no zoned GC logic).
	 */
	if (c.zenfs_aux_mode) {
		int i;
		c.zoned_mode = 0;
		...
		MSG(0, "Info: ZenFS aux mode - formatting rzone as conventional SSD\n");
	}
	...
	if (c.conf_reserved_sections_bb && c.zoned_mode) {
		MSG(0, "\tError: This Reserved area only can be specified on non zoned device\n");
		goto err_format;
	}
