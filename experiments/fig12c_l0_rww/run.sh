#!/bin/bash
# Paper Fig.12c — L0 placement under readwhilewriting: 10M-key fillrandom
# prefill, then 16 readers + 1 writer for 150 s, dual instances, j=8.
# Same three configurations as Fig.12a/b; the fixed-CNS run dies during the
# prefill (expected), giving the truncated curve.
set -euo pipefail
source "$(dirname "$0")/../common.sh"
need_root; need_femu
cd "$ROOT/hyznsd"; need_bin "$ROOT/rocksdb/db_bench_zns"; need_bin "$ROOT/rocksdb/db_bench_l0cns"; need_bin ./hyznsd
...
echo "done -> hyznsd/$OUT (analysis/ = read CDF+throughput, ravg/ = Fig.12c curves)"
