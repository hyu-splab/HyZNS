#!/bin/bash
# Paper Fig.13 — ZTree vs Hy-ZTree insert throughput on the FEMU HYHOSTSSD
# device, thread scaling 4/8/16/32/64/128/256:
#   ZTree     ZNS-only (whole device sequential, r_end=0)
#   Hy-ZTree  internal nodes on F2FS-over-dm-hyhost CNS, leaves spill to ZNS,
#             hyhostd grows the CNS on demand (coord=ctree 2-phase handshake)
