...
// HYSSD
struct nvme_hyssd_rzone_report {
	__le64 nr_rzones;
};

/* Zone modification report structure */
struct nvme_hyssd_victim_idx {
    __le32 old_idx;
    __le32 new_idx;
};

struct nvme_hyssd_modify_zone_report {
    __le64 nr_victims;              /* Number of zones/pages that need to be relocated */
    __u8   victim_type;             /* 0: zone-level relocation, 1: page-level relocation */
    __u8   rsvd[7];                 /* Reserved for 8-byte alignment */
    struct nvme_hyssd_victim_idx entries[]; /* Array of victim indices using existing struct */
};
...
enum nvme_zns_recv_action {
	...
	NVME_HYSSD_REPORT_VICTIM = 0X20,
	NVME_HYSSD_REPORT_RZONE = 0X21,
};
