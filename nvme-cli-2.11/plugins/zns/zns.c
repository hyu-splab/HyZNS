...
static int modify_zone(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
    const char *desc = "Modify the number of random zones in the namespace";
    const char *nr_rzone = "New number of random zones to set";
    const char *verbose = "Show detailed zone modification information";
    ...
    OPT_ARGS(opts) = {
        OPT_UINT("namespace-id", 'n', &cfg.namespace_id, namespace_id),
        OPT_UINT("nr-rzone", 'r', &cfg.nr_rzones, nr_rzone),
	...
	if (cfg.nr_rzones == 0) {
		printf("Number of R-zones must be greater than 0\n");
        return errno;
	}

    /* Step 1: Get victim information using zone management receive */
	printf("modify-zone command called\n");
    __u32 data_len = sizeof(struct nvme_hyssd_modify_zone_report) + 
                     4096 * sizeof(struct nvme_hyssd_victim_idx); // Max 4K entries
    struct nvme_hyssd_modify_zone_report *report = calloc(1, data_len);
    if (!report) {
        err = -ENOMEM;
        goto close_dev;
    }

    struct nvme_zns_mgmt_recv_args recv_args = {
        .args_size = sizeof(recv_args),
        .fd = dev_fd(dev),
        .nsid = cfg.namespace_id,
        .zra = NVME_HYSSD_REPORT_VICTIM,
        .data = report,
        .data_len = data_len,
        .timeout = NVME_DEFAULT_IOCTL_TIMEOUT,
		.nrz = cfg.nr_rzones,
    ...
    err = nvme_zns_mgmt_recv(&recv_args);
    if (err) {
        fprintf(stderr, "Failed to get zone modification info: %s\n", 
                nvme_status_to_string(err, false));
        goto free_data;
    }

	printf("received victim report\n");
	printf("number of victims: %llu\n", report->nr_victims);
	printf("victim type: %u\n", report->victim_type);

    /* Step 2: Perform modification using zone management send */
    struct nvme_zns_mgmt_send_args send_args = {
        .args_size = sizeof(send_args),
        .fd = dev_fd(dev),
        .nsid = cfg.namespace_id,
        .zsa = NVME_HYSSD_MODIFY_ZONE,
        .data = report,
        .data_len = data_len,
        .timeout = NVME_DEFAULT_IOCTL_TIMEOUT,
		.nrz = cfg.nr_rzones,
    ...
    err = nvme_zns_mgmt_send(&send_args);
    if (err) {
        fprintf(stderr, "Failed to perform zone modification: %s\n",
                nvme_status_to_string(err, false));
        goto free_data;
    }

    if (cfg.verbose) {
        printf("Zone modification completed:\n");
        printf("  Number of relocated entries: %u\n", 
               le64_to_cpu(report->nr_victims));
        printf("  Relocation type: %s\n", 
               report->victim_type ? "Page-level" : "Zone-level");
        
        printf("  Relocations performed:\n");
        for (unsigned i = 0; i < le64_to_cpu(report->nr_victims); i++) {
            printf("    %u -> %u\n", 
                   le32_to_cpu(report->entries[i].old_idx),
                   le32_to_cpu(report->entries[i].new_idx));
        }
    } else {
        printf("Successfully modified number of random zones to %u\n",
               cfg.nr_rzones);
    }
...
static int report_rzone(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
	const char *desc = "Report the number of R-Zones";
	...
	__u32 data_len = sizeof(struct nvme_hyssd_rzone_report);
	struct nvme_hyssd_rzone_report *report = calloc(1, data_len);
	if (!report) {
		err = -ENOMEM;
		goto close_dev;
	}
	...
	err = nvme_zns_mgmt_recv(&recv_args);
	if (err) {
		fprintf(stderr, "Failed to get the number of R-zones: %s\n", nvme_status_to_string(err, false));
		goto free_data;
	}
	
	printf("Number of R-zones: %llu\n", report->nr_rzones);
