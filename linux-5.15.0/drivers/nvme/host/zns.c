		...
		struct nvme_command *c, enum nvme_zone_mgmt_action action)
{
	...
	if (req_op(req) == REQ_OP_ZONE_RESET_ALL)
		c->zms.select_all = 1;
	/* REQ_OP_ZONE_MODIFY now uses the standard SLBA mapping above:
	 * bi_sector (= new ABA LBA = new first S-zone's zslba) flows
	 * straight into c->zms.slba, consistent with the other ZSAs. */
	return BLK_STS_OK;
}

blk_status_t nvme_setup_zone_mgmt_recv(struct nvme_ns *ns, struct request *req,
		struct nvme_command *c, enum nvme_zone_mgmt_action action)
{
	c->zmr.opcode = nvme_cmd_zone_mgmt_recv;
	c->zmr.nsid = cpu_to_le32(ns->head->ns_id);
	c->zmr.slba = 0;
	c->zmr.numd = cpu_to_le32(sizeof(__u32) / 4 - 1);  /* u32 payload, in dwords - 1 */
	c->zmr.zra = action;
	c->zmr.zrasf = 0;
	c->zmr.pr = 0;
