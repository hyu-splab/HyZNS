...
#self.set_cpus(ncore)
    ...
    def mount_f2fs(self, media, fs, mnt_path):
        return True
    ...
    def fxmark(self, media, fs, bench, ncore, nfg, nbg, dio):
        ...
        # 2) periodic_output 옵션이 켜져 있으면 바이너리에 플래그 추가
        if getattr(self, 'periodic_output', False):
            cmd_parts.append("--periodic-output")
        ...
        # periodic_output 모드면 stdout 그대로, 아니면 PIPE로 캡처
        if self.periodic_output:
            # 실시간 출력: stdout 그대로 터미널에 연결
            p = self.exec_cmd(cmd, None)
        else:
            # 기본 모드: PIPE 로 받고, 끝난 뒤에만 로그에 기록
            p = self.exec_cmd(cmd, self.redirect)
            for line in p.stdout:
                self.log(line.decode("utf-8").strip())
...
if __name__ == "__main__":
    ...
    parser.add_option("--periodic_output", action="store_true", help="Enable periodic output every second during the benchmark")
    ...
    run_config = [
        ...
        (Runner.CORE_FINE_GRAIN, PerfMon.LEVEL_LOW,("nvme", "f2fs", "DWOL", "16", "directio")),
        #(Runner.CORE_FINE_GRAIN, PerfMon.LEVEL_LOW,("nvme", "f2fs", "DWAL", "16", "directio")),
...
#confirm_media_path()
    for c in run_config:
        ...
        runner.periodic_output = opts.periodic_output  # 주기적 출력 옵션 전달
        runner.run()
