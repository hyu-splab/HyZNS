...
#define BLOCKS_PER_PAGE (PAGE_SIZE / BLOCK_SIZE)  // = 8
...
static int main_work(struct worker *worker)
{
	...
	int fd, rc = 0;
	uint64_t iter = 0;
	fprintf(stdout, "dwol [512B] main_work start\n");
	...
	// 	goto err_out;
		for (int i = 0; i < BLOCKS_PER_PAGE; i++) {
            // 512B 단위로 offset = 0에 계속 덮어씀
            if (pwrite(fd, page + i * BLOCK_SIZE, BLOCK_SIZE, 0) != BLOCK_SIZE)
                goto err_out;

			if (bench->periodic_output) {
				// periodic_output이 활성화되었을 때만 원자적 카운팅
				atomic_fetch_add(&worker->atomic_works, 1);
			}
        }
	}
out:
