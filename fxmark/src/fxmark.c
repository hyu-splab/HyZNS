...
static int parse_option(int argc, char *argv[], struct cmd_opt *opt)
{
	static struct option options[] = {
        ...
        {"periodic-output", no_argument, 0, 'p'},
		{0,           0,                 0, 0},
	};
	int arg_cnt;
    ...
    opt->periodic_output    = 0;
	for(arg_cnt = 0; 1; ++arg_cnt) {
		int c, idx = 0;
		c = getopt_long(argc, argv, 
				"t:n:g:d:D:r:b:e:l:p", options, &idx);
		if (c == -1)
			break; 
		switch(c) {
		case 't':
...
static void usage(FILE *out)
{
	...
	fprintf(out, "  --periodic-output   = stat interval 1sec\n");
...
static void init_bench(struct bench *bench, struct cmd_opt *opt)
{
    ...
    bench->periodic_output = opt->periodic_output;
