...
#
# Both runs use the IDENTICAL dm-hyzns stack so only the WAL path differs:
#
#     RocksDB / db_bench  (fillrandom)
#               |
#     ZenFS (S-region, ZNS)  +  aux F2FS (R-region, via dm-hyzns L2P)
#               |
#     /dev/mapper/hyzns0   (R = R_ZONES GiB, S = rest;  ABA STATIC — no modify)
#               |
...
DMHYZNS=../dm-hyzns
source "$DMHYZNS/scripts/_lib.sh"            # reset_device_full_r, HYZNS_ZONE_SECTORS
...
run_variant() {                               # <label> <binary>
    ...
    echo "[$label] setup: dm-hyzns R=${R_ZONES}GiB/S=rest, aux F2FS + ZenFS"
    setup_topology
    ...
    ZENFS_L0_ON_AUX=0 "$bin" \
        ...
        &> "$log"
    # dm-hyzns R-region counters (proof: WAL hits R only under POSIX)
