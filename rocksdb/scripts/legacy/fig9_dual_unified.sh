...
#   <pfx>.dmstatus.log  dm-hyhost R-FTL series, prefixed with `date +%s.%N` (epoch.ns)
...
ZS=$HYHOST_ZONE_SECTORS; ZENFS=./plugin/zenfs/util/zenfs
...
setup(){  # $1 maxprov(0/1); fresh dm + aux F2FS + 2 ZenFS instances. nonzero on any failure.
  ...
  lsmod|awk '{print $1}'|grep -qx dm_hyhost || "$DMHYHOST/scripts/load.sh">/dev/null 2>&1
  reset_device_full_r "$BACKING" >/dev/null 2>&1||true
  if [[ $1 == 1 ]]; then
    echo "0 $SZ hyhost $BACKING $((R_ZONES*ZS)) $((R_MAX*ZS))" | dmsetup create "$DMNAME" || return 1   # 5th arg=max_r_end=R_MAX (dm hard cap on grow)
    dmsetup message "$DMNAME" 0 set_r_end $((R_ZONES*ZS)) || return 1
    mkfs.f2fs -f -m -H -A -P "$DEV" >/dev/null 2>&1 || return 1
  else
    echo "0 $SZ hyhost $BACKING $((R_ZONES*ZS))" | dmsetup create "$DMNAME" || return 1
    dmsetup message "$DMNAME" 0 set_r_end $((R_ZONES*ZS)) || return 1
