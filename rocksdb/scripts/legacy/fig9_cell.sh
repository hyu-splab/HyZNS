...
# Same dm-hyhost stack everywhere; only the data placement differs.
#
...
ZENFS=./plugin/zenfs/util/zenfs; ZS=$HYHOST_ZONE_SECTORS
...
# ---- setup dm-hyhost R-region + aux F2FS + ZenFS ----
...
if [[ $MAXPROV == 1 ]]; then
  echo "0 $SZ hyhost $BACKING $((R_ZONES*ZS)) $SZ" | dmsetup create "$DMNAME"   # 5th arg = max_r_end (whole dev) for online grow
