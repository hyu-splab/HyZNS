...
#     MODE=hyzns  FEMU HyZNS       : ZenFS on the S-region of dm-hyzns
...
#            and for hyzns a buildable dm-hyzns (../dm-hyzns).
set -uo pipefail
...
# below can follow it. It reuses the zns code path (raw ZBD, no dm-hyzns) but
...
# A HyZNS namespace AND a pure-ZNS device BOTH report
# queue/zoned=host-managed, so aba_ok() (vendor ABA report, in _fig_lib.sh) is
# what tells them apart: only HyZNS answers it.
if [[ -z ${MODE:-} ]]; then            # auto: probe ABA on each zoned nvme
    for _d in $(list_zoned_nvme); do
        ...
        ZNS_DEV=${ZNS_DEV:-$_d}        # first non-HyZNS zoned dev = pure ZNS
    done
    [[ -z ${MODE:-} && -n ${ZNS_DEV:-} ]] && MODE=zns
    [[ -z ${MODE:-} ]] && die "no host-managed zoned nvme found (is FEMU running?)"
fi
BACKING=${BACKING:-/dev/nvme0n1}       # hyzns: FEMU HyZNS namespace
...
if (( REAL )); then
    ...
    ! aba_ok "$ZNS_DEV" || echo "WARN: $ZNS_DEV answers the ABA report — looks like HyZNS, not pure ZNS (MODE=hyzns?)"
else
    [[ $AUX_DEV == "$BACKING" ]] && die "AUX_DEV == BACKING ($AUX_DEV)"
    [[ -b $BACKING ]] || die "backing device $BACKING not found (set BACKING=...)"
    aba_ok "$BACKING" || die "MODE=hyzns but $BACKING does not answer the ABA report — is the HyZNS (femu_mode=5) FEMU running?"
fi
...
dm_prepare_once() {                    # build (if stale) + reload the .ko
    ...
    if [[ ! -f $DMHYZNS/dm-hyzns.ko ]] \
       || [[ -n $(find "$DMHYZNS" -maxdepth 1 \( -name '*.c' -o -name '*.h' \) -newer "$DMHYZNS/dm-hyzns.ko" 2>/dev/null) ]]; then
        echo "==> rebuilding dm-hyzns.ko"
        "$DMHYZNS/scripts/build.sh" || die "dm-hyzns build failed"
    fi
...
setup_dev() {                          # fresh ZenFS for a write invocation
    ...
    if [[ $MODE != hyzns ]]; then      # zns and real: ZenFS on the raw ZBD
        ...
        local zs=$HYZNS_ZONE_SECTORS sz total last dmdev
