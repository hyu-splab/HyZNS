...
cd ../../../../dm-hyzns && KDIR=../linux-5.15.0 ./scripts/build.sh
```
...
via the vendor ABA report — only HyZNS (femu_mode=5) answers it, which
...
results accumulate. Modes: `zns` (whole zoned nvme) / `hyzns` (dm-hyzns
...
The aux F2FS lives on the CNS of the SAME device (dm-hyzns R-region); ZenFS
