...
bool LevelCompactionBuilder::PickIntraL0Compaction() {
  // HYSSD: env toggle to disable intra-L0 (L0->L0) compaction. On the L0-on-CNS
  // setup intra-L0 merges small flush SSTs into huge (100s of MB) L0 files that
  // bloat the small CNS area; disabling it keeps L0 = lean flush files.
  if (getenv("ROCKSDB_NO_INTRA_L0")) return false;
  start_level_inputs_.clear();
  const std::vector<FileMetaData*>& level_files =
