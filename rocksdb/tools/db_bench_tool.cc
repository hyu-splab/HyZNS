...
class ReporterAgent {
 public:
  ReporterAgent(Env* env, const std::string& fname,
    ...
    if (!s.ok()) {
      // Non-fatal: the stall-immune throughput is also emitted to stderr
      // (===REPORT), which is what the log parsers read, so keep going.
      fprintf(stderr, "ReporterAgent: can't open %s (%s); using stderr ===REPORT only\n",
  ...
  // thread safe. op_type: 0=kRead, 1=kWrite (OperationType enum, passed as int
  // because that enum is defined after this class). Splits the interval counter
  // so ===REPORT can emit per-second read and write throughput separately.
  void ReportFinishedOps(int64_t num_ops, int op_type) {
    total_ops_done_.fetch_add(num_ops);
    if (op_type == 0) read_ops_done_.fetch_add(num_ops);         // kRead
    else if (op_type == 1) write_ops_done_.fetch_add(num_ops);   // kWrite
  }

  // thread safe. Records the per-op completion latency (us) so ===REPORT can emit the
  // per-interval MAX read/write latency -- the rigorous, per-request stall (the analog
  // of fio's clat-max), used to measure how long the ZenFS co-tenant is blocked.
  void ReportLatency(int op_type, uint64_t us) {
    std::atomic<uint64_t>* m = (op_type == 0) ? &max_rd_lat_us_
                             : (op_type == 1) ? &max_wr_lat_us_ : nullptr;
    if (!m) return;
    uint64_t cur = m->load();
    while (us > cur && !m->compare_exchange_weak(cur, us)) {}
  }
 ...
 private:
  ...
  void SleepAndReport() {
    ...
    // Sub-second reporting override: ROCKSDB_REPORT_INTERVAL_MS lets a resize
    // experiment sample ops at e.g. 100ms so a ~2-5s modify dip is well resolved.
    // Default keeps the integer-second behaviour (report_interval_seconds*1000).
    uint64_t iv_ms = report_interval_secs_ * 1000ULL;
    if (const char* e = getenv("ROCKSDB_REPORT_INTERVAL_MS")) {
      uint64_t v = (uint64_t)atoll(e); if (v > 0) iv_ms = v;
    }
    while (true) {
      {
        std::unique_lock<std::mutex> lk(mutex_);
        if (stop_ ||
            stop_cv_.wait_for(lk, std::chrono::milliseconds(iv_ms),
                              [&]() { return stop_; })) {
          // stopping
          break;
        }
      ...
      auto read_ops_done_snapshot = read_ops_done_.load();
      auto write_ops_done_snapshot = write_ops_done_.load();
      uint64_t max_rd_lat = max_rd_lat_us_.exchange(0);  // per-interval max op latency, then reset
      uint64_t max_wr_lat = max_wr_lat_us_.exchange(0);
      ...
      // Stall-immune throughput to stderr: this background thread fires on a
      // timed wait, so it records interval_qps even when the foreground
      // benchmark is stalled (qps=0) -> no gaps in the series.
      fprintf(stderr,
              "\n%.6f ===REPORT %" PRId64 " %" PRId64 " %" PRId64 " %" PRIu64 " %" PRIu64 "\n",
              clock->NowMicros() / 1000000.0,
              static_cast<int64_t>(total_ops_done_snapshot - last_report_),
              static_cast<int64_t>(read_ops_done_snapshot - last_read_report_),
              static_cast<int64_t>(write_ops_done_snapshot - last_write_report_),
              max_rd_lat, max_wr_lat);
      if (report_file_) {
        ...
        if (!s.ok()) {
          fprintf(stderr, "Can't write report file (%s); stderr ===REPORT continues\n",
                  s.ToString().c_str());
          report_file_.reset();
        }
      }
      last_report_ = total_ops_done_snapshot;
      last_read_report_ = read_ops_done_snapshot;
      last_write_report_ = write_ops_done_snapshot;
    }
  }
  ...
  std::atomic<int64_t> read_ops_done_;
  std::atomic<int64_t> write_ops_done_;
  std::atomic<uint64_t> max_rd_lat_us_{0};   // per-interval max read op latency (us)
  std::atomic<uint64_t> max_wr_lat_us_{0};   // per-interval max write op latency (us)
  int64_t last_report_;
  int64_t last_read_report_;
  int64_t last_write_report_;
...
// Per-op latency trace: phase counter + current benchmark name, set once per
// benchmark at the ===PHASE_START site (single thread, before workers spawn) and
// read by each Stats::Stop() to name its dump file. See ROCKSDB_OPLAT_DIR.
static std::atomic<int> g_oplat_phase{0};
static std::string g_oplat_bench;
...
class Stats {
 private:
  ...
  // Per-op latency trace (ROCKSDB_OPLAT_DIR): buffer every op's (ts,lat,op) in
  // RAM and dump to one CSV per thread at Stop(). Off unless the env is set.
  bool oplat_on_ = false;
  std::string oplat_dir_;
  std::vector<uint64_t> oplat_ts_;   // completion timestamp (us, epoch)
  std::vector<uint32_t> oplat_us_;   // per-op latency (us)
  std::vector<uint8_t>  oplat_op_;   // OperationType (0=read,1=write,...)
 ...
 public:
  ...
  void Start(int id) {
    ...
    // per-op latency trace: enable + pre-reserve so the hot-path push_back never
    // reallocates (keeps measurement perturbation negligible).
    const char* d = getenv("ROCKSDB_OPLAT_DIR");
    oplat_on_ = (d && *d);
    oplat_dir_ = oplat_on_ ? d : "";
    oplat_ts_.clear(); oplat_us_.clear(); oplat_op_.clear();
    if (oplat_on_) {
      oplat_ts_.reserve(1u << 20); oplat_us_.reserve(1u << 20);
      oplat_op_.reserve(1u << 20);
    }
  }
  ...
  void Stop() {
    ...
    // Dump this thread's per-op latency trace AFTER stamping finish_, so the
    // (potentially multi-second) write is excluded from the measured seconds_.
    // One file per (phase, benchmark, thread); ts lets the plotter window-tag ops.
    if (oplat_on_ && !oplat_ts_.empty()) {
      char path[1024];
      snprintf(path, sizeof path, "%s/oplat_p%d_%s_t%d.csv", oplat_dir_.c_str(),
               g_oplat_phase.load(), g_oplat_bench.c_str(), id_);
      FILE* f = fopen(path, "w");
      if (f) {
        std::vector<char> buf(1u << 20);
        setvbuf(f, buf.data(), _IOFBF, buf.size());
        fputs("ts_us,lat_us,op\n", f);
        for (size_t i = 0; i < oplat_ts_.size(); ++i) {
          fprintf(f, "%" PRIu64 ",%u,%u\n", oplat_ts_[i], oplat_us_[i],
                  (unsigned)oplat_op_[i]);
        }
        fclose(f);
      } else {
        fprintf(stderr, "oplat: can't open %s\n", path);
      }
    }
  }
  ...
  void FinishedOps(DBWithColumnFamilies* db_with_cfh, DB* db, int64_t num_ops,
    ...
    if (reporter_agent_) {
      reporter_agent_->ReportFinishedOps(num_ops, static_cast<int>(op_type));
    }
    if (FLAGS_histogram) {
      ...
      if (reporter_agent_) reporter_agent_->ReportLatency(static_cast<int>(op_type), micros);
      if (oplat_on_) {
        oplat_ts_.push_back(now);
        oplat_us_.push_back(micros > 0xFFFFFFFFull ? 0xFFFFFFFFu
                                                   : static_cast<uint32_t>(micros));
        oplat_op_.push_back(static_cast<uint8_t>(op_type));
      }
    ...
    if (done_ >= next_report_ && FLAGS_progress_reports) {
      if (!FLAGS_stats_interval) {
        ...
        if (FLAGS_stats_interval_seconds &&
          ...
          fprintf(stderr,
                  "%.6f %s thread_%d: %" PRIu64 ",%" PRIu64
                  " ops_and "
                  "%.1f,%.1f ops/second_in %.6f,%.6f seconds\n",
                  now / 1000000.0, clock_->TimeToStringMicros(now).c_str(), id_,
                  done_ - last_report_done_, done_, // interval done, total done
                  (done_ - last_report_done_) / (usecs_since_last / 1000000.0), // interval ops/s
                  done_ / ((now - start_) / 1000000.0), // total_ops/total_sec
                  (now - last_report_finish_) / 1000000.0, // interval seconds
                  (now - start_) / 1000000.0); // total seconds
  ...
  void Report(const Slice& name) {
    ...
    std::string extra;
    double elapsed = (finish_ - start_) * 1e-6; // total elapsed seconds
    if (bytes_ > 0) {
...
class Benchmark {
 private:
  ...
  int64_t writes_;
  // true only while a benchmark with an explicit [N<n>] op-count override is
  // running: the count is a HARD target, so DoWrite must NOT let --duration
  // (set for a later readwhilewriting phase in the same process) cut the fill
  // short. See the [N<n>] swap site and DoWrite's test_duration.
  bool bench_count_override_ = false;
  int64_t readwrites_;
  int64_t merge_keys_;
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run(ToolHooks& hooks) {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      // YCSB value size fixed at 400 (= field_count * field_len)
      field_count_ = 10;
      field_prefix_ = "field";
      int field_len = 100;
      ...
      int num_repeat = 1;
      int num_warmup = 0;
      int64_t bench_writes = -1;   // per-benchmark op-count override ([N<n>])
      if (!name.empty() && *name.rbegin() == ']') {
        ...
        while (std::getline(args_stream, bench_arg, '-')) {
          ...
          } else if (bench_arg[0] == 'N') {
            // Override this benchmark's op count (e.g. overwrite[N10000000]
            // after fillrandom fills --num keys). Lets one process do a deep
            // FR fill + a shorter OW without a DB reopen (which the l0cns
            // aux-L0 path cannot survive). Applies to write benches via
            // writes_ (DoWrite: num_ops = writes_).
            bench_writes = std::stoll(bench_arg.substr(1));
          }
        }
      }
      ...
      if (method != nullptr) {
        ...
        // [N<n>] op-count override: swap writes_ for this benchmark only.
        int64_t saved_writes = writes_;
        if (bench_writes >= 0) { writes_ = bench_writes; bench_count_override_ = true; }
        CombinedStats combined_stats;
        for (int i = 0; i < num_repeat; i++) {
          Stats stats;
          if (name == "YCSBLOAD" || name == "fillrandom" || name == "fillseq") {
            num_threads = 1;
        ...
        writes_ = saved_writes;   // restore after the [N<n>] override
        bench_count_override_ = false;
      }
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    // Official, epoch-stamped workload-phase marker (parsed for FR/OW boundaries).
    // Leading \n guarantees its own line (prior bench output may lack a trailing newline).
    fprintf(stderr, "\n%.6f ===PHASE_START %s\n",
            FLAGS_env->NowMicros() / 1000000.0, name.ToString().c_str());

    // per-op latency trace: advance the phase tag (set before workers spawn so
    // each Stats::Stop() names its dump oplat_p<phase>_<bench>_t<id>.csv).
    g_oplat_phase.fetch_add(1);
    g_oplat_bench = name.ToString();
  ...
  // no multi-db support; single column family
  void DoYCSB(ThreadState *thread, YCSB_MODE mode) {
    if (mode == YCSB_LOAD) {
      ...
      while (!duration.Done(1)) {
        ...
        if (!s.ok()) {
          fprintf(stdout, "ycsb put error (%s)\n", s.ToString().c_str());
          ErrorExit();
        }
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    // An explicit [N<n>] count is a hard target; ignore --duration for it so a
    // shared --duration meant for a later phase cannot truncate the fill.
    const int test_duration =
        (write_mode == RANDOM && !bench_count_override_) ? FLAGS_duration : 0;
...
// optimization, so skip the check of the max level.
  ...
  void BGWriter(ThreadState* thread, enum OperationType write_merge) {
    ...
    RandomGenerator gen;
    int64_t bytes = 0;
    uint64_t bg_start_us = FLAGS_env->NowMicros();
    ...
    thread->stats.AddBytes(bytes);
    // The writer thread is excluded from the merged report (its stats carry
    // SetExcludeFromMerge), so the readwhilewriting summary line only shows
    // reads. Emit the write side in the same summary format for fig7 parsing.
    uint64_t bg_elapsed_us = FLAGS_env->NowMicros() - bg_start_us;
    if (written > 0 && bg_elapsed_us > 0) {
      fprintf(stdout,
              "%-12s : %11.3f micros/op %ld ops/sec; %6.1f MB/s "
              "(%u writes by the background writer thread)\n",
              "rww-writes",
              static_cast<double>(bg_elapsed_us) / written,
              static_cast<long>(written / (bg_elapsed_us * 1e-6)),
              (bytes / 1048576.0) / (bg_elapsed_us * 1e-6), written);
      fflush(stdout);
    }
  }
