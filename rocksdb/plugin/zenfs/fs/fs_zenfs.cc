...
Status Superblock::DecodeFrom(Slice* input) {
  ...
  GetFixed64(input, &reserved_zone_count_);
...
void Superblock::EncodeTo(std::string* output) {
  ...
  PutFixed64(output, reserved_zone_count_);
...
void Superblock::GetReport(std::string* reportString) {
  ...
  if (reserved_zone_count_ > 0) {
    reportString->append("\nReserved Zones (HYSSD):\t\t");
    reportString->append(std::to_string(reserved_zone_count_));
  }
...
ZenFS::~ZenFS() {
  ...
  // Stop Zone Arbiter before the GC worker so no resize runs during shutdown.
  if (zone_arbiter_) {
    Info(logger_, "Stopping Zone Arbiter");
    zone_arbiter_->Stop();
    zbd_->SetZoneArbiter(nullptr);
    zone_arbiter_.reset();
  }
...
void ZenFS::GCWorker() {
  while (run_gc_worker_) {
    ...
    for (const auto& zone : snapshot.zones_) {
      // Victim ANY written zone (open / closed-partial / full), not only
      // capacity==0: non-full zones hold garbage too ("ghost garbage").
      // garbage% is measured against WRITTEN bytes only, so a zone's
      // unwritten tail is never counted as garbage (that would
      // false-victimize open zones).
      uint64_t written = zone.max_capacity - zone.capacity;
      if (written == 0) continue;
      uint64_t garbage = written > zone.used_capacity ?
                         written - zone.used_capacity : 0;
      uint64_t garbage_percent_approx = 100 * garbage / written;
...
IOStatus ZenFS::DeleteFileNoLock(std::string fname, const IOOptions& options,
  ...
  if (zoneFile != nullptr) {
    ...
    if (zoneFile->wal_type == 2) {
      // fprintf(stderr, "wal file delete\n");
    ...
    if (zoneFile->l0_aux == 1) {
      // close may fail (fd=-1 when the create itself failed under ENOSPC) —
      // still unlink, or the dead file survives and pins F2FS space forever.
      if (close(zoneFile->fd) != 0)
        fprintf(stderr, "close error : %s (errno %d)\n",
                zoneFile->filename.c_str(), errno);

      if (unlink(zoneFile->filename.c_str()) != 0) {
        fprintf(stderr, "unlink error : %s\n", zoneFile->filename.c_str());
        return IOStatus::IOError();
      }
      fprintf(stderr, "[ZL0-] delete(unlink) aux L0 SST: %s\n", zoneFile->filename.c_str());
    }
...
IOStatus ZenFS::OpenWritableFile(const std::string& filename,
  ...
  bool resetIOZones = false;
  {
    ...
    if (ends_with(fname, ".log")) {
      ...
      int fd = open(ToAuxPath(file_name).c_str(), O_DIRECT | O_RDWR | O_CREAT | O_SYNC, 0644);
      zoneFile->fd = fd;
      ...
      zoneFile->l0_aux = 0;
      // Non-SST, non-WAL files (MANIFEST/CURRENT/OPTIONS/IDENTITY) are small but
      // MANIFEST stays open the whole DB lifetime. Place them in high zones so
      // the low absorb range stays pure-SST (migratable) for online CNS grow.
      if (!ends_with(fname, ".sst")) zoneFile->meta_high = true;
#ifdef L0_POSIX
      std::string file_name = filename.substr(1, filename.length());
      zoneFile->filename = ToAuxPath(file_name);
#endif
...
Status ZenFS::Mount(bool readonly) {
  ...
  */
  ...
  uint64_t reserved = superblock_->GetReservedZoneCount();
  if (reserved > 0) {
    zbd_->SetReservedZones(reserved);
    Info(logger_, "Restored reserved zone count from superblock: %lu\n", reserved);
  }
  ...
  if (!readonly) {
    ...
    // Start Zone Arbiter (auto-modify) only when explicitly opted in via
    // ZENFS_ZONE_ARBITER=1, so ordinary experiments are unaffected. Requires
    // HYSSD mode (reserved zones > 0) and an aux (F2FS) path.
    uint64_t arbiter_reserved = superblock_->GetReservedZoneCount();
    const char* arb_env = getenv("ZENFS_ZONE_ARBITER");
    bool arb_enabled =
        arb_env && (arb_env[0] == '1' || arb_env[0] == 'o' || arb_env[0] == 'O');
    if (arb_enabled && arbiter_reserved > 0 &&
        !superblock_->GetAuxFsPath().empty()) {
      Info(logger_, "Starting Zone Arbiter for dynamic CNS resizing");
      ZoneArbiterConfig arbiter_config;
      arbiter_config.aux_path = superblock_->GetAuxFsPath();
      arbiter_config.min_rzone_count = static_cast<uint32_t>(arbiter_reserved);
      // Env overrides for tuning (e.g. aggressive rescue under fast L0 fill).
      if (const char* e = getenv("ZENFS_ARBITER_INTERVAL_MS"))
        arbiter_config.check_interval_us = (uint64_t)atoll(e) * 1000ULL;
      if (const char* e = getenv("ZENFS_ARBITER_COOLDOWN_MS"))
        arbiter_config.adjustment_cooldown_us = (uint64_t)atoll(e) * 1000ULL;
      if (const char* e = getenv("ZENFS_ARBITER_THRESHOLD"))
        arbiter_config.f2fs_expand_threshold = atof(e);
      if (const char* e = getenv("ZENFS_ARBITER_MAXZONES"))
        arbiter_config.max_zones_per_adjustment = (uint32_t)atoi(e);
      if (const char* e = getenv("ZENFS_ARBITER_FREE_FLOOR"))
        arbiter_config.free_floor = (uint32_t)atoi(e);
      if (const char* e = getenv("ZENFS_ARBITER_STEP"))
        arbiter_config.grow_step = (uint32_t)atoi(e);
      if (const char* e = getenv("ZENFS_ARBITER_MAX_RZONE"))
        arbiter_config.max_rzone_count = (uint32_t)atoi(e);
      // Lower the shrink floor below the initial reservation (Fig8(b) shrink
      // latency sweep needs to release zones below the start point).
      if (const char* e = getenv("ZENFS_ARBITER_MIN_RZONE"))
        arbiter_config.min_rzone_count = (uint32_t)atoi(e);
      // External-driver mode: hyhostd owns the policy; the arbiter only
      // publishes status + executes absolute-target resize requests.
      if (const char* e = getenv("ZENFS_ARBITER_EXTERNAL"))
        arbiter_config.external = (e[0] == '1' || e[0] == 'o' || e[0] == 'O');
      zone_arbiter_.reset(new ZoneArbiter(this, zbd_, logger_, arbiter_config));
      zbd_->SetZoneArbiter(zone_arbiter_.get());
      IOStatus arbiter_status = zone_arbiter_->Start();
      if (arbiter_status.ok()) {
        run_zone_arbiter_ = true;
      } else {
        Warn(logger_, "Failed to start Zone Arbiter: %s",
             arbiter_status.ToString().c_str());
        zbd_->SetZoneArbiter(nullptr);
        zone_arbiter_.reset();
      }
    }
...
Status ZenFS::MkFS(std::string aux_fs_p, uint32_t finish_threshold,
  ...
  super.SetReservedZoneCount(zbd_->GetReservedZones());
...
Status NewZenFS(FileSystem** fs, const ZbdBackendType backend_type,
  ...
  while (getline(ss, sbuffer, '/')) { // split on "/"
  ...
  IOStatus zbd_status = zbd->Open(false, false, true, start_zone, end_zone, 
...
IOStatus ZenFS::MigrateExtents( const std::vector<ZoneExtentSnapshot*>& extents, uint64_t *copied_data, bool sst_only, uint64_t min_dest_start) {
  IOStatus s;
  ...
  for (auto* ext : extents) {
    ...
    // GC migrates only SST extents; CNS grow (sst_only=false) migrates all file
    // types so the to-be-absorbed S-zones are fully emptied for the S->R move.
    if (!sst_only || ends_with(fname, ".sst")) {
      file_extents[fname].emplace_back(ext);
    }
  }

  for (const auto& it : file_extents) {
    s = MigrateFileExtents(it.first, it.second, copied_data, min_dest_start);
    if (!s.ok()) break;
...
IOStatus ZenFS::MigrateFileExtents(
    ...
    uint64_t *copied_data, uint64_t min_dest_start) {
  ...
  // Closed file: hold the WR lock to prevent write reopens while we migrate.
  // OPEN-for-write file (WR lock is held for its whole open lifetime, e.g.
  // MANIFEST/LOG): do NOT skip it — a skipped extent in the absorb range wedged
  // GrowCNS in a BUSY loop forever. Instead exclude the writer through
  // writer_mtx_ (every zenfs append path takes it) and, if its ACTIVE extent
  // sits in an absorbing zone (idle writer never self-evicts), seal it so the
  // zone can drain; the sealed extent migrates on the next drain pass.
  bool wr_locked = zfile->TryAcquireWRLock();
  std::unique_lock<std::mutex> mig_lk;
  if (!wr_locked) {
    mig_lk = std::unique_lock<std::mutex>(zfile->WriterMtx());
    IOStatus fs_ = zfile->SealForAbsorb();
    if (!fs_.ok())
      Warn(logger_, "MigrateFileExtents: seal of open file %s failed: %s",
           fname.c_str(), fs_.ToString().c_str());
  }
  ...
  if (wr_locked) zfile->ReleaseWRLock();
