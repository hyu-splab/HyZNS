...
Status ZoneFile::MergeUpdate(std::shared_ptr<ZoneFile> update, bool replace) {
  ...
  for (long unsigned int i = 0; i < update_extents.size(); i++) {
    ...
    if (zone == nullptr) { // WAL_CNS
      extents_.push_back(new ZoneExtent(extent->start_, extent->length_, nullptr));
    } else {
...
IOStatus ZoneFile::SealForAbsorb() {
  if (active_zone_ == nullptr || !active_zone_->IsAbsorbing())
    return IOStatus::OK();
  PushExtent();
  IOStatus s = active_zone_->Finish();
  if (!s.ok()) return s;
  return CloseActiveZone();
}

IOStatus ZoneFile::CloseWR() {
  std::lock_guard<std::mutex> mig_lg(writer_mtx_);
  IOStatus s;
...
IOStatus ZoneFile::AllocateNewZone() {
  Zone* zone;
  IOStatus s = zbd_->AllocateIOZone(lifetime_, io_type_, &zone, meta_high);

  // fprintf(stderr, "Num concurrent alloc : %u\n", zbd_->num_concurrent_alloc_.load());
  // fprintf(stderr, "open zone token : %lu\n", zbd_->open_io_zones_.load());
...
IOStatus ZoneFile::BufferedAppend(char* buffer, uint32_t data_size) {
  ...
  while (left) {
    /* CNS-grow: if our active zone entered the absorb range, finish it and roll
     * off to a fresh zone (F2FS curseg-eject analog) so the absorb zone has no
     * writer and GrowCNS can migrate+reset it without an append race. */
    if (active_zone_->IsAbsorbing()) {
      s = active_zone_->Finish();
      if (!s.ok()) return s;
      s = CloseActiveZone();
      if (!s.ok()) return s;
      s = AllocateNewZone();
...
IOStatus ZoneFile::SparseAppend(char* sparse_buffer, uint32_t data_size) {
  ...
  zbd_->wal_writes_num++;
  uint64_t wal_start = zbd_->env_->NowMicros();
  ...
  if (wal_type == 1 || wal_type == 2) {
    while (left) {
      ...
      if (wal_type == 1) {
        ...
        if (ret < 0) {
          fprintf(stdout, "ZoneFile::SparseAppend::write failed %d\n", errno);
          return IOStatus::IOError();
        }
      }
    ...
    zbd_->wal_writes_time += zbd_->env_->NowMicros() - wal_start;
  ...
  // debug logging for WAL files only (filter by file name):
  // fprintf(stderr, "WAL SparseAppend start: file=%s data_size=%u \n",
  //                  GetFilename().c_str(), data_size);

  while (left) {
    /* CNS-grow: roll off an absorbing active zone (see BufferedAppend). */
    if (active_zone_->IsAbsorbing()) {
      s = active_zone_->Finish();
      if (!s.ok()) return s;
      s = CloseActiveZone();
...
IOStatus ZoneFile::Append(void* data, int data_size) {
  ...
  IOStatus s = IOStatus::OK();
  uint64_t sst_start = zbd_->env_->NowMicros();

  zbd_->sst_writes_num++;

  // L0 file on aux_path. Loop to handle SHORT writes: write() may return fewer
  // bytes than requested (e.g. near an F2FS boundary); an unchecked single
  // write() would leave the on-disk file shorter than file_size_ and RocksDB
  // would fail with "Sst file size mismatch".
  if (l0_aux == 1) {
      char* p = (char*)data;
      uint32_t remaining = left;
      while (remaining > 0) {
        ssize_t ret = write(fd, p, remaining);
        if (ret < 0) {
          if (errno == EINTR) continue;
          fprintf(stderr, "ZoneFile::Append::write failed %d %d\n", fd, errno);
          // carry errno into the status so the caller (db_bench "put error")
          // shows the real cause instead of a bare "IO error:" (28=ENOSPC).
          return IOStatus::IOError("aux-L0 write " + filename + " errno " +
                                   std::to_string(errno));
        }
        if (ret == 0) {  // no progress (e.g. out of space)
          fprintf(stderr, "ZoneFile::Append::write no progress %d\n", fd);
          return IOStatus::IOError("aux-L0 write " + filename +
                                   " no progress (out of space)");
        }
        p += ret;
  ...
  while (left) {
    /* CNS-grow: also roll off if the active zone entered the absorb range. */
    if (active_zone_->capacity_ == 0 || active_zone_->IsAbsorbing()) {
      PushExtent();

      /* finish an absorbing not-full zone so CloseActiveZone returns its
       * active-zone token (and no further writes land on it). */
      if (active_zone_->capacity_ != 0 && active_zone_->IsAbsorbing()) {
        s = active_zone_->Finish();
        if (!s.ok()) return s;
      }
  ...
  zbd_->sst_writes_time += zbd_->env_->NowMicros() - sst_start;
...
IOStatus ZonedWritableFile::DataSync() {
  ...
  if (zoneFile_->l0_aux == 1) {
    // No O_SYNC on the L0 fd; make the data durable here (the sync point),
    // matching ZNS which flushes the zone on DataSync rather than per write.
    if (fdatasync(zoneFile_->fd) < 0) {
      return IOStatus::IOError("l0_aux fdatasync failed: " +
                               std::to_string(errno));
    }
    return IOStatus::OK();
  }

  if (zoneFile_->wal_type == 2 || zoneFile_->wal_type == 1) {
    buffer_mtx_.lock();
    s = FlushBuffer();
    buffer_mtx_.unlock();
    if (!s.ok()) {
...
IOStatus ZonedWritableFile::Append(const Slice& data,
  ...
  // ZoneArbiter (auto-modify) write pause: quiesce while the CNS area resizes.
  ZoneArbiter* arbiter = zoneFile_->GetZbd()->GetZoneArbiter();
  if (arbiter) {
    arbiter->WaitIfPaused();
    arbiter->IncrementPendingWriters();
  }
  ...
  if (arbiter) {
    arbiter->DecrementPendingWriters();
  }
...
IOStatus ZonedSequentialFile::PositionedRead(uint64_t offset, size_t n,
  ...
  if (zoneFile_->l0_aux == 1) {
    ...
    size_t left = n;
    char* ptr = scratch;
    uint64_t fadv_off = offset;
    while (left > 0) {
      r = pread(zoneFile_->fd, ptr, left, static_cast<off_t>(offset));
      if (r <= 0) {
        if (r == -1 && errno == EINTR) {
          continue;
        }
    ...
    if (r < 0) {
      ...
      s = IOStatus::IOError("While pread " + std::to_string(n) + " bytes from offset " +
                      std::to_string(offset) + " " + std::to_string(errno),
                  zoneFile_->filename);
    } else {
      posix_fadvise(zoneFile_->fd, static_cast<off_t>(fadv_off), n,
                    POSIX_FADV_DONTNEED);
    }
...
IOStatus ZonedRandomAccessFile::Read(uint64_t offset, size_t n,
  ...
  if (zoneFile_->l0_aux == 1) {
    ...
    if (r < 0) {
      // An error: return a non-ok status
      s = IOStatus::IOError(
          std::to_string(zoneFile_->fd) + " While pread offset " +
              std::to_string(offset) + " len " + std::to_string(n) +
              " " + std::to_string(errno),
          zoneFile_->filename);
    } else {
      // Evict the just-read pages so the L0 fd keeps no page-cache advantage
      // over the ZNS path (which is O_DIRECT and never cached).
      posix_fadvise(zoneFile_->fd, static_cast<off_t>(fadv_off), n,
                    POSIX_FADV_DONTNEED);
    }
    *result = Slice(scratch, (r < 0) ? 0 : n - left);
