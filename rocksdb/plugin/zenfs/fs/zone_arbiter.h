...
struct ZoneArbiterConfig {
  ...
  // In zones (1 GiB) — a dm-hyhost line is 64 MiB, so the trigger converts
  ...
  // External-driver mode (hyhostd): the in-process threshold decider is gated
...
class ZoneArbiter {
 ...
 private:
  ...
  // hyhostd contract: publish <aux>/.hyzns_status (R_Z = free_zones) and execute
  ...
  // dm-hyhost R-region physical state (via `dmsetup status`). Returns true and
  ...
  bool QueryDmHyhostLines(uint64_t* lines, uint64_t* free_lines,
 ...
 private:
  ...
  // dm-hyhost no-space counter from the previous check (rising == grow now)
