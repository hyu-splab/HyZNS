...
void ZoneArbiter::MonitorLoop() {
  ...
  while (!stop_requested_.load()) {
    ...
    // hyznsd contract: publish current zone status every poll so the daemon's
    // fs R_Z source can read it (<aux>/.hyzns_status).
    if (config_.publish_status) PublishStatus();

    // hyznsd contract: execute an absolute-target resize request if present.
    ...
    // External-driver mode: hyznsd owns the policy. Skip the in-process
...
// hyznsd contract: publish ZenFS zone status atomically (temp + rename) so the
// daemon's `rz_source=fs` reader gets a consistent snapshot. Format matches
// hyznsd's parser (key=value lines; it reads `free_zones`).
...
// hyznsd contract: a control file `<aux>/.hyzns_resize` holding an ABSOLUTE
...
bool ZoneArbiter::HandleResizeReq() {
  ...
  printf("[ZoneArbiter] hyznsd resize req: R %lu -> %u\n", cur, target);
  fflush(stdout);
  ...
  printf("[ZoneArbiter] hyznsd resize %s -> R=%lu\n", verdict, result_r);
  fflush(stdout);
...
// Read dm-hyzns physical line state via `dmsetup status`. The aux F2FS is
...
// dm-hyzns write-amplifies F2FS writes, so it can be physically exhausted
// (free_lines=0 -> nospc/EIO) while F2FS still thinks it has logical free space.
// dm name comes from ZENFS_DM_HYZNS_NAME (default "hyzns0").
bool ZoneArbiter::QueryDmHyznsLines(uint64_t* lines, uint64_t* free_lines,
                                     uint64_t* free_pages,
                                     uint64_t* zone_pblocks, uint64_t* nospc) {
  const char* nm = getenv("ZENFS_DM_HYZNS_NAME");
  std::string name = nm ? nm : "hyzns0";
...
IOStatus ZoneArbiter::QueryF2FSStats(F2FSStats* stats) {
  ...
  // Prefer the dm-hyzns physical free-capacity as the expand trigger; fall
  // back to (max-prov-skewed) statvfs only if dm status is unreadable.
  // A dm-hyzns line is 64 MiB but a zone is 1 GiB (=16 lines), so the raw
  ...
  uint64_t dev_lines = 0, free_lines = 0, free_pages = 0, zone_pblocks = 0,
           nospc = 0;
  if (QueryDmHyznsLines(&dev_lines, &free_lines, &free_pages, &zone_pblocks,
                         &nospc)) {
    ...
    // genuinely few free zones remain, OR when dm-hyzns's nospc counter rises
    ...
    printf("[ZoneArbiter] dm-hyzns R: R0=%lu free_zones=%lu (free_lines=%lu) "
...
IOStatus ZoneArbiter::ShrinkCNS(uint32_t count) {
  ...
  // (f2fs tail-drain + freeze + checkpoint + dm-hyzns R-tail force-GC + device ABA)
