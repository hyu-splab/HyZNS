...
IOStatus ZbdlibBackend::CheckScheduler() {
  ...
  std::string buf;
  getline(f, buf);
  /* Bio-based device-mapper targets (dm-hyhost) expose no scheduler
   * ("none" with no alternatives); zone write ordering is enforced by
   * the backing device's mq-deadline queue instead. */
  if (buf.find("[mq-deadline]") == std::string::npos && buf != "none") {
    f.close();
...
IOStatus ZbdlibBackend::Open(bool readonly, bool exclusive,
                             ...
                             unsigned int *max_open_zones, bool allow_shared) {
  zbd_info info;

  /* The non-direct file descriptor acts as an exclusive-use semaphore */
  fprintf(stdout, "Open() exclusive=%d, allow_shared=%d\n", exclusive, allow_shared);
  if (exclusive && !allow_shared) {
    /* hyssd shared mode (allow_shared) opens without O_EXCL below */
    // read_f_ = zbd_open(filename_.c_str(), O_RDONLY , &info);
    read_f_ = zbd_open(filename_.c_str(), O_RDONLY | O_EXCL, &info);
  } else {
    read_f_ = zbd_open(filename_.c_str(), O_RDONLY, &info);
  }
  read_f_conv_ = zbd_open("/dev/nvme0n1", O_RDONLY, &info);
  ...
  read_direct_f_conv_ = zbd_open("/dev/nvme0n1", O_RDONLY | O_DIRECT, &info);
  ...
  if (readonly) {
    ...
    write_f_conv_ = zbd_open("/dev/nvme0n1", O_WRONLY | O_DIRECT, &info);
  ...
  reserved_zone_count_ = info.nr_rzones;
